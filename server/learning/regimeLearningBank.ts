...
import {
  ...
  QUARANTINED_LABELS,
  CF_LABELS,
  R6_MIN_RESOLVED_SAMPLE_FOR_PROMOTION,
  round,
  avg,
  pct,
  freqTop,
  returnR,
  scoreDataQuality,
  sourceConfidenceOfCase,
  sourceConfidenceOfCounterfactual,
  phaseForShadowCase,
  phaseForCounterfactual,
  counterfactualReturnR,
  conditionTags,
  isResolvedLabel,
  isQuarantinedCase,
  isPendingOpenCase,
...
export function collectRegimeLearningBank(input: CollectRegimeLearningInput = {}): RegimeLearningBank {
  ...
  const backfillDryRun = regimeUnknownRepairDryRun({
    ...
    reconstructionLogEntries: input.reconstructionLogEntries,
    now,
  } as any);
  ...
  const regimeSnapshotReconstructionAttemptedDates = backfillDryRun.snapshotReconstructionAttemptedDates ?? [];
  const regimeSnapshotReconstructionSucceededDates = backfillDryRun.snapshotReconstructionSucceededDates ?? [];
  ...
  return {
    ...
    regimeSnapshotReconstructionSucceededDates,

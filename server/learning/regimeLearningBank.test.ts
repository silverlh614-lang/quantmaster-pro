...
describe('Regime Learning Bank', () => {
  ...
  it('reduces true UNKNOWN after reconstructed daily regime snapshot recovery', () => {
    ...
    expect(bank.regimeSnapshotReconstructionSourceBreakdown.REGIME_RESOLVER_DECISION_LOG).toBe(1);
    ...
    expect(bank.regimeSourceInventoryAuditStatus).toBe('PRIORITY_SOURCE_AVAILABLE');
    expect(bank.regimeSourceInventoryByDate['2026-05-13'].regimeResolverDecisionLog).toBe(1);
    expect(bank.regimeSourceInventoryMissingSources['2026-05-13']).not.toContain('regimeResolverDecisionLog');
    expect(bank.regimeSnapshotReconstructionPriorityDate).toBe('2026-05-13');
    expect(bank.priorityDateReconstructionStatus).toBe('SUCCESS');
    expect(bank.priorityDateRecoveredSampleCount).toBe(1);
    expect(bank.priorityDateFailureReason).toBe('NONE');
    expect(bank.postReconstructionTrueUnknownRatio).toBe(bank.trueUnknownRatio);
    expect(bank.regimePromotionStillBlocked).toBe(true);
    expect(bank.regimePromotionBlockReason).toBe('REGIME_PROMOTION_BLOCKED_TRUE_UNKNOWN_RATIO_HIGH');
  });
});

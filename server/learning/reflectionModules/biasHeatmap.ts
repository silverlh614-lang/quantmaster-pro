/**
 * biasHeatmap.ts — Bias Heatmap (#11).
 *
...
function scoreLossAversion(i: BiasHeatmapInputs): BiasScore {
  const losses = i.closedToday.filter((t) => (t.returnPct ?? 0) < 0).length;
  const approachAlerts = i.activePositions.filter((t) => (t.stopApproachStage ?? 0) >= 2).length;
  const score = clamp01(losses >= 2 && approachAlerts >= 1 ? 0.7 + approachAlerts * 0.1 : losses * 0.2);
  return {
    bias: 'LOSS_AVERSION',
    score: Number(score.toFixed(2)),
    evidence: `손실 ${losses}건, 손절선 접근 ${approachAlerts}건`,
  };
}
...
function scoreOverconfidence(i: BiasHeatmapInputs): BiasScore {
  const wins = i.closedToday.filter((t) => t.status === 'HIT_TARGET').length;
  ...
  return {
    bias: 'OVERCONFIDENCE',
    score: Number(score.toFixed(2)),
    evidence: `익절 ${wins}건, 슬롯 ${i.availableSlots}개`,
  };
}

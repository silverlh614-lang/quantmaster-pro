...
export function collectLearningPulseV3(now: Date = new Date()) {
  ...
  return { repairLastRun: { closed: last?.close?.closed ?? 0, outcomeFinalized: last?.outcome?.finalized ?? 0, attributionProcessed: last?.attr?.processedCount ?? 0, diagnosticSuggest: last?.suggest?.proposals?.length ?? 0, criticalIntegrityEvents: last?.criticalIntegrityEvents ?? 0, brokerOrdersCreated: last?.brokerOrdersCreated ?? last?.close?.brokerOrdersCreated ?? 0 }, outcomeQuality: { win: outcome.WIN, loss: outcome.LOSS, expired: outcome.EXPIRED, quarantined: outcome.QUARANTINED, expectancyR: outcome.expectancyR, dataQualityScore: outcome.dataQualityScore }, cohortSummary: { freshShadowCount: cohort.freshShadowCount, backlogRepairCount: cohort.backlogRepairCount, recoveredMetadataCount: cohort.recoveredMetadataCount, freshExpectancyR: (cohort.expectancyByCohort as any).FRESH_SHADOW ?? 0, backlogExpectancyR: (cohort.expectancyByCohort as any).BACKLOG_REPAIR ?? 0 }, metadataRepairStatus: { missingTargetStop: metadataRepairStatus.missingTargetStop, missingEntryPrice: metadataRepairStatus.missingEntryPrice, quarantined: metadataRepairStatus.quarantined }, openResidue: { openRemaining: open.openRemaining, closeCandidates: open.closeCandidates, quarantined: open.quarantined, zeroCloseReasonTop3: open.zeroCloseReasonTop3 }, attributionQuality: { samples7d: attr.samples7d, conditionCoverage: attr.conditionCoverage, overfitRisk: attr.overfitRisk, lowSampleConditions: attr.lowSampleConditions.length, unstableConditions: attr.unstableConditions.length }, suggest: { proposals: suggest.totalProposals, autoApply: false as const, counterfactualSampleSize: suggest.sampleSize }, holdingOutlierStatus: { outlierCount: holding.outlierCount, excludedFromPromotion: holding.excludedFromPromotion }, promotionMetricBasis: outcome.promotionMetricBasis, gemini: { nextScheduledAt: gemini?.nextScheduledAt, recommendationOnly: true as const }, executionImpact: 'NONE' as const };
}

  ...
  for (const entry of validEntries) {
    ...
    for (const horizon of ALL_HORIZONS) {
      ...
      try {
        ...
        if (result.available) {
          // entryPrice 부재 — DATA_UNAVAILABLE
          const entryPriceCandidate = (entry as { entryPrice?: number }).entryPrice;
          const returnPct = safeReturnPct(entryPriceCandidate, result.price);

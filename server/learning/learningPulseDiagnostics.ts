...
import { inspectCounterfactualMetadataHealth } from '../persistence/attributionMetadataRepair.js';
...
// Patch 2 gap (d): counterfactual metadataHealth 결손 판정 + missingRatio 순수 helper (read-only 진단, 동작 보존).
function evaluateMetadataHealthFlag(m: { counterfactualSamples: number; missingEither: number }): { repairNeeded: boolean; missingRatio: number } {
  const hasSamples = m.counterfactualSamples > 0;
  const rawRatio = hasSamples ? m.missingEither / m.counterfactualSamples : 0;
  return { repairNeeded: hasSamples && rawRatio > 0.5, missingRatio: hasSamples ? parseFloat(rawRatio.toFixed(3)) : 0 };
}
export function collectLearningPulseV2(now: Date = new Date()) {
  ...
  // Patch 2 gap (d): counterfactual/shadow lane stop/target 결손 read-only 진단.
  // 데이터를 수정하지 않는다 (repair 는 별도 호출). 학습 경로 무영향.
  const metadataHealth = inspectCounterfactualMetadataHealth();
  const metadataFlag = evaluateMetadataHealthFlag(metadataHealth);
  ...
  // Patch 2 gap (d): counterfactual stop/target 결손 과반 → repair 권고 (read-only 진단 flag).
  if (metadataFlag.repairNeeded) flags.push('counterfactual_metadata_repair_needed');
  return { capturedAt: now.toISOString(), ghost: { open: openCases.length, closed7d, closed14d, closed30d, closeRate7d, closeRateDenominator7d, lastGhostCloseRunCloseRate: lastClose && lastClose.candidatesScanned > 0 ? Math.min(1, lastClose.closed / lastClose.candidatesScanned) : undefined, staleOpenCount, oldestOpenAge }, closeResolver: { lastRunAt: lastClose?.lastRunAt, candidatesScanned: lastClose?.candidatesScanned ?? 0, closed: lastClose?.closed ?? 0, pendingRetry: lastClose?.pendingRetry ?? 0, quarantined: lastClose?.quarantined ?? 0 }, attribution: { samples7d: attr.filter(r => recent(r.closedAt, 7, now)).length, samples14d: attr.filter(r => recent(r.closedAt, 14, now)).length, samples30d: attr.filter(r => recent(r.closedAt, 30, now)).length, target: LEARNING_ATTRIBUTION_TARGET_7D, starvationReason: starvation.primaryReason === 'resolved' ? 'NONE' : starvation.primaryReason, previousStarvationReason: starvation.previousStarvationReason, metadataHealth: { counterfactualSamples: metadataHealth.counterfactualSamples, missingStop: metadataHealth.missingStop, missingTarget: metadataHealth.missingTarget, missingEither: metadataHealth.missingEither, missingRatio: metadataFlag.missingRatio } }, conditionWeights: { learned, unlearned: Math.max(0, totalWeights - learned), skipped: starvation.reasons.data_quarantine_too_many, changed: learned, lastUpdateAt: undefined }, suggest: { total7d: totalSuggest, byChannel, blocker: proposals.at(-1)?.blocker ?? (totalSuggest === 0 ? 'NO_DIAGNOSTIC_PROPOSAL' : 'NONE'), diagnosticSuggestCreated: proposals.some(p => recent(p.createdAt, 7, now)) }, gemini: { callsThisMonth: budget.callCount, tradingDaysThisMonth: LEARNING_TRADING_DAYS_PER_MONTH, utilizationRate: budget.callCount / LEARNING_TRADING_DAYS_PER_MONTH, lastCallAt: budget.lastReflectionDate, nextScheduledAt: geminiLast?.nextScheduledAt }, diagnosticFlags: flags };
}
export function formatLearningPulseV2Message(s: ReturnType<typeof collectLearningPulseV2>): string {
  return [`🩺 Learning Pulse v2`, `Ghost Portfolio: OPEN ${s.ghost.open} / closed7d ${s.ghost.closed7d} / closed14d ${s.ghost.closed14d} / closed30d ${s.ghost.closed30d} / closeRate7d ${(s.ghost.closeRate7d*100).toFixed(1)}% / lastGhostCloseRunCloseRate ${typeof s.ghost.lastGhostCloseRunCloseRate === 'number' ? `${(s.ghost.lastGhostCloseRunCloseRate*100).toFixed(1)}%` : 'N/A'} / staleOpenCount ${s.ghost.staleOpenCount} / oldestOpenAge ${s.ghost.oldestOpenAge}m`, `Close Resolver: lastRunAt ${s.closeResolver.lastRunAt ?? 'N/A'} / scanned ${s.closeResolver.candidatesScanned} / closed ${s.closeResolver.closed} / pendingRetry ${s.closeResolver.pendingRetry} / quarantined ${s.closeResolver.quarantined}`, `Attribution: samples7d ${s.attribution.samples7d} / samples14d ${s.attribution.samples14d} / samples30d ${s.attribution.samples30d} / target ${s.attribution.target} / starvationReason ${s.attribution.starvationReason}${s.attribution.previousStarvationReason ? ` / previousStarvationReason ${s.attribution.previousStarvationReason}` : ''}`, `Attribution Metadata Health: counterfactualSamples ${s.attribution.metadataHealth.counterfactualSamples} / missingStop ${s.attribution.metadataHealth.missingStop} / missingTarget ${s.attribution.metadataHealth.missingTarget} / missingEither ${s.attribution.metadataHealth.missingEither} / missingRatio ${(s.attribution.metadataHealth.missingRatio*100).toFixed(1)}% (read-only)`,  `Condition Weights: learned ${s.conditionWeights.learned} / unlearned ${s.conditionWeights.unlearned} / skipped ${s.conditionWeights.skipped} / changed ${s.conditionWeights.changed}`, `Suggest: total7d ${s.suggest.total7d} / byChannel ${JSON.stringify(s.suggest.byChannel)} / blocker ${s.suggest.blocker} / diagnosticSuggestCreated ${s.suggest.diagnosticSuggestCreated}`, `Gemini: callsThisMonth ${s.gemini.callsThisMonth} / tradingDaysThisMonth ${s.gemini.tradingDaysThisMonth} / utilizationRate ${(s.gemini.utilizationRate*100).toFixed(0)}% / lastCallAt ${s.gemini.lastCallAt ?? 'N/A'} / nextScheduledAt ${s.gemini.nextScheduledAt ?? 'N/A'}`, `Diagnostic Flags: ${s.diagnosticFlags.join(', ') || 'none'}`].join('\n');
}

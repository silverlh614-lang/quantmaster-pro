...
} from './sectorCanonicalReturnLookup.js'; // ADR-0577 Stage 1: canonical(L1) 우선 데이터원 룩업 (의존성-0 leaf, KIS/fs 부작용 0, flag default OFF=null).
...
export function normalizeSectorThemeCycleForGate2(
  ...
  // ADR-0577 D1: 우선순위 raw → canonical(L1) → basket(diagnostic) fallback.
  ...
  // ADR-0577 D1: rank 도 raw → canonical(L1) → basket fallback. flag OFF → canonical null → basket(byte-identical).
  ...
  // ADR-0577 D1: leading 도 raw → canonical(L1) → basket fallback. flag OFF → canonical null → basket(byte-identical).

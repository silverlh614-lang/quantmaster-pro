 ...
 *   7. SECTOR_INDEX_MASTER 본체 변경 0 (alias 등록은 별도 PR).
 ...
 *   2. 다중 공백 → 단일 공백
 *   3. 괄호 () [] {} 내부 + 괄호 자체 제거
 *   4. 특수문자 (`·`, `/`, `,`, `&`) 제거 + 양쪽 공백 재 trim
 *   5. 코스피/코스닥 접두/접미 제거 (KOSPI / 코스피 / KOSDAQ / 코스닥 — 단어 단위)
 *   6. lower-case 변환 (alias 매칭은 lower-case)
 *
...
export function normalizeIndexNameForLookup(
  ...
  // 특수문자 제거 (간격유지를 위해 공백으로 치환)
  normalized = normalized.replace(/[·\/,&]/g, ' ');
  // 코스피/코스닥 접두·접미 제거 (단어 단위)
  normalized = normalized.replace(/(KOSPI|KOSDAQ|코스피|코스닥)/gi, ' ');
  // 다중 공백 → 단일 공백
  normalized = normalized.replace(/\s+/g, ' ').trim();
  // lower-case (대소문자 무관 매칭)
 ...
 *   4. indexCodeSource='STOCK_DAILY_DERIVED' → STOCK_DAILY
 *   5. 그 외 (indexCode 부재 + source 미상) → UNKNOWN
 */
 ...
 *   6. expandAliasCandidates(normalized).length >= 2 → AMBIGUOUS_ALIAS
 *   7. tryDirectNameLookup(raw) 매칭 → null (이미 backfill 되었어야 함, 분류 안 함)
 *   8. 정규화 후 매칭 → NORMALIZATION_MISMATCH (정상은 backfilledByAliasExpansion)
 *   9. (정규화 후 매칭 없음) → ALIAS_NOT_REGISTERED 또는 UNKNOWN_INDEX_NAME
 */

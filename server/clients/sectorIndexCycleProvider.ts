...
import { publishCanonicalSectorReturns } from './sectorCanonicalReturnLookup.js'; // ADR-0572 Stage 0: canonical 룩업 leaf 에 publish (read-only, 신규 fetch 0).
...
/** 테스트 전용 — 캐시 리셋 (ADR-0572 canonical leaf 스냅샷 동반 리셋). */
...
export async function getSectorCycleReturns(
  ...
  if (_cache && now - _cache.fetchedAtMs < CACHE_TTL_MS) {
    // ADR-0572 Stage 0: 캐시 hit 시에도 canonical leaf 에 publish (신규 fetch 0, read-only 재사용).
  ...
  // ADR-0572 Stage 0: 빌드 결과를 canonical leaf 에 publish (per-candidate 축이 룩업으로 소비).

...
export function buildSectorEnergyAudit(input: {
  ...
  return {
    ...
    sourceTierBreakdown: {
      KIS_OFFICIAL_INDEX: input.sourceTier === 'KIS_OFFICIAL_INDEX' ? 1 : 0,
      KIS_OFFICIAL_DAILY: input.sourceTier === 'KIS_OFFICIAL_DAILY' ? 1 : 0,
      KIS_STOCK_BASKET_DERIVED: input.sourceTier === 'KIS_STOCK_BASKET_DERIVED' ? 1 : 0,
      KRX_INDEX: input.sourceTier === 'KRX_INDEX' ? 1 : 0,
      KRX_ETF: input.sourceTier === 'KRX_ETF' ? 1 : 0,
      YAHOO_GLOBAL_PROXY: input.sourceTier === 'YAHOO_GLOBAL_PROXY' ? 1 : 0,
      YAHOO_PROXY: input.sourceTier === 'YAHOO_PROXY' ? 1 : 0,
      STOCK_DAILY_FALLBACK: input.sourceTier === 'STOCK_DAILY_FALLBACK' ? 1 : 0,
      INTERNAL_PROXY: input.sourceTier === 'INTERNAL_PROXY' ? 1 : 0,
      MISSING: input.sourceTier === 'MISSING' ? 1 : 0,
    },

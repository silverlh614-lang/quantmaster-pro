...
// ADR-0587 — public type declarations extracted to ./kisSectorEnergyProvider/types.ts
// (byte-equivalent, validate:complexity 1500 한계 자연 통과). 외부 consumer 호환 위해 re-export.
import type {
  KisSectorEnergySourceTier,
  KisSectorEnergyLeadershipConfidence,
  KisSectorEnergyCoverageBreakdown,
  KisSectorBasketRow,
  KisSectorEnergyIndexRow,
  SectorIndexVerificationStatus,
  KisSectorIndexDryRunErrorClass,
  KisSectorIndexDryRunRow,
  KisSectorIndexDryRunReport,
  SectorIdxcodeMasterRow,
  SectorIndexCodeMasterVerificationResult,
  KisSectorEnergyProviderOverrides,
  KisSectorEnergyProviderResult,
} from './kisSectorEnergyProvider/types.js';
export type {
  KisSectorEnergySourceTier,
  KisSectorEnergyLeadershipConfidence,
  KisSectorEnergyCoverageBreakdown,
...
export function formatKisSectorIndexDryRunSection(report: KisSectorIndexDryRunReport): string {
  ...
  const lines = [
    ...
    `legacySourceTierDiagnosticOnly: <code>${report.sourceTier}</code>`,
    ...
    `legacySectorBoostAllowedDiagnosticOnly: <b>${String(report.sectorBoostAllowed)}</b>`,
    `legacyStrongBuyAllowedDiagnosticOnly: <b>${String(report.strongBuyAllowed)}</b>`,
...
function resultFromInputs(
  ...
  return {
    ...
    reasons,
    diagnostics: [
      `legacySourceTierDiagnosticOnly=${sourceTier}`,
      `validSectorCount=${validSectorCount}/${TOTAL_SECTOR_COUNT}`,
      `legacyLeadershipConfidenceDiagnosticOnly=${leadershipConfidence}`,
      'officialSource=KIS_API',
      'officialIndex=false',
      'legacySectorBoostAllowedDiagnosticOnly=false',
      'legacyStrongBuyAllowedDiagnosticOnly=false',

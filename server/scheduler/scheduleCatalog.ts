...
export const SCHEDULE_CATALOG: ScheduleEntry[] = [
  ...
  { timeKst: '18:00', label: 'After-hours runtime debug snapshot capture (Patch-AFTER-HOURS-RUNTIME-DEBUG-SNAPSHOT-001/002/003)', group: 'maintenance', jobName: 'runtime_debug_snapshot_capture', silentWhen: 'RUNTIME_DEBUG_SNAPSHOT_DISABLED=true 이거나 KRX 휴장일이면 무음 — 18:00 = 장 종료 +2.5h (Patch-003: 17:00 시점 직전 scan 미완료 결함 차단), scan+learning+reflection cron 모두 완료 후. replay 시 SELL_ONLY 의미 부여 금지 (sessionInterpretation=INTRADAY_REPLAY_EQUIVALENT). 다음 거래일 18:00 성공 capture 시 single-latest overwrite, 금→월 lifecycle 보존.' },

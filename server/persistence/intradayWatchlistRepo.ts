 ...
 *
 * Pre-Market 워치리스트와 분리 운영되며, 장중 돌파·거래량 급증·수급 매집 종목을
 * 실시간 발굴하여 등록한다. 즉시 매수 금지 — 15분 경과 후 재검증 통과 시에만
 * signalScanner가 진입을 허용한다.
 *
 * 발굴 경로:
 *   BREAKOUT       — 거래량 2× + 등락률 +1.5% + 20일 고점 돌파 (손절 -5%)
 *   SUPPLY_DEMAND  — 거래량 2.5× + 양봉 + MA20 위 + 눌림목 셋업 (손절 -4%)
 *   PULLBACK       — 눌림목 반등 초입 (손절 -4%)
 */
export interface IntradayWatchlistEntry {
  ...
  stopLoss: number;          // 절대가 손절선 (돌파형 -5% / 수급형·눌림목형 -4%)
  targetPrice: number;       // 목표가 (진입가의 10% 상승)
  sector?: string;           // 섹터 정보
  /** 발굴 경로: 돌파형 / 수급형 / 눌림목형 */
  entryPath?: 'BREAKOUT' | 'SUPPLY_DEMAND' | 'PULLBACK';
  /** 진입 준비 완료: 15분 경과 + 가격 강도 재검증 통과 시 true */

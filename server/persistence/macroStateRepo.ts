...
export interface MacroState {
  ...
  /** ADR-0590: R6 trigger 봉의 KRX 거래일(YYYY-MM-DD KST). age-only freshness 오판 차단용. */
  kospiTriggerSourceTradeDate?: string;
  /** ADR-0590: 오늘 KOSPI 현재가 vs 전일종가 수익률(%). KIS 종합지수(L1) intraday. flag OFF 시 미설정. */
  kospiIntradayReturn?: number;
  /** ADR-0590: kospiIntradayReturn 소스 거래일(YYYY-MM-DD KST) — 오늘 아니면 recovery 미사용. */
  kospiIntradaySourceTradeDate?: string;
  /** ADR-0590: kospiIntradayReturn 마지막 KIS fetch 성공 시각(ISO). carry-forward 시 갱신 안 됨. */

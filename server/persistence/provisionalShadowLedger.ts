...
export interface ProvisionalShadowLedgerEntry {
  ...
  createdAtKst: string;
  /**
   * ADR-0620 — measurement entry reference price at provisional-shadow record time.
   ...
   */
  entryPrice?: number;
  /** ADR-0620 — entryPrice 출처. KIS_CURRENT = buyListLoop currentPrice (실진입 게이트 동일 소스, ADR-0561 정합). */
...
function buildEntry(input: RecordProvisionalShadowInput, nowIso: string): ProvisionalShadowLedgerEntry {
  return {
    ...
    // ADR-0620 — measurement entryPrice/source additive stamp (candidate 에서만, 소급 backfill 0).

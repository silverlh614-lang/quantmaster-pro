...
export interface R6TriggerBreakdown {
  ...
  /** ADR-0592: 트리거 봉의 KRX 거래일(YYYY-MM-DD KST). */
  kospiTriggerSourceTradeDate?: string;
  /** ADR-0592: 트리거 봉 거래일이 오늘 KRX 거래일인가. false면 intraday 트리거 강등. */
  tradeDateIsToday?: boolean;
  /** ADR-0592: recovery 평가에 오늘 intraday 수익률을 사용했는가(진단). */
...
export interface RegimeTransitionState {
  ...
  /**
   * ADR-0664 — 레짐 값 히스테리시스(effectiveRegime 디바운스) 보류 후보·관측 상태.
  ...
  return {
    ...
    consecutiveHealthyRecoveryTicks:
      typeof record.consecutiveHealthyRecoveryTicks === "number" && Number.isFinite(record.consecutiveHealthyRecoveryTicks)
        ...
        : 0,
    // ADR-0664 — 히스테리시스 보류 상태. legacy json/부재 → undefined·0 (flag OFF 미소비).

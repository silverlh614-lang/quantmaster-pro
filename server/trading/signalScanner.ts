...
export async function runAutoSignalScan(options?: { sellOnly?: boolean; forceBuyCodes?: string[] }): Promise<{ positionFull?: boolean }> {
  ...
  if (vixGating.noNewEntry) {
    ...
    // ADR-0104 — 장시작/장마감 윈도우(09:00~10:00 / 15:00~16:00)만 발송.
    // 그 외 시간(예: 14:30 FOMC DAY 청산)에는 도배 차단.
  ...
  // ADR-0080: 자본 가중 슬롯 회계 — 70% 익절 잔존 30% 포지션 5개 = 1.5 슬롯 점유 환산.
  // 기존 단순 카운트(activeSwingCount.length)는 잔존 자본을 반영 못해 신규 진입 슬롯 누수.
  // SLOT_CAPITAL_WEIGHTED_DISABLED=true env 시 단순 카운트 동치 (긴급 롤백).

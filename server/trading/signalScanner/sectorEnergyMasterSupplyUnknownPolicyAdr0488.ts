...
function sectorIndexCoveragePolicyLines(
  ...
  return [
    ...
    `SectorIndexPromotionReadinessDiagnosticOnly: officialTargetSectorCount=${officialTargetSectorCount} internalGroupedSectorCount=${internalGroupedSectorCount} safePromotionEligibleSectorCount=${safePromotionEligibleSectorCount} unsafeAliasSectorCount=${unsafeAliasSectorCount} unresolvedSectorCount=${unresolvedSectorCount} verifiedSuccessCount=${verifiedSuccessCount} verifiedCoverageByOfficialTarget=${pct(verifiedCoverageByOfficialTarget)} verifiedCoverageByInternalGrouped=${pct(verifiedCoverageByInternalGrouped)} verifiedCoverageExcludingUnsafeAlias=${pct(verifiedCoverageExcludingUnsafeAlias)} selectedPromotionMetricDiagnosticOnly=${readiness?.selectedPromotionMetric ?? selectedMetric} selectedPromotionCoverageDiagnosticOnly=${pct(selectedCoverageValue)} requiredPromotionCoverageDiagnosticOnly=${pct(requiredPromotionCoverage)} qualityUsableCoverageByOfficialTarget=${pct(qualityUsableCoverageByOfficialTarget)} qualityUsableCoverageExcludingUnsafeAlias=${pct(qualityUsableCoverageExcludingUnsafeAlias)} legacyPromotionAllowedDiagnosticOnly=${effectivePromotionAllowed} reason=${selectedReadinessReason} internalGroupedMetricWouldPass=${internalGroupedMetricWouldPass} safeOnlyMetricWouldPass=${safeOnlyMetricWouldPass} safeOfficialTargetCount=${safeOfficialTargetCount} safeOfficialVerifiedCount=${safeOfficialVerifiedCount} safeOfficialVerifiedCoverage=${pct(safeOfficialVerifiedCoverage)} unsafeExcludedCount=${unsafeExcludedCount} unsafeExcludedNames=${unsafeExcludedNames.join(',') || 'NONE'} officialTargetVerifiedCoverageDiagnostic=${pct(officialTargetVerifiedCoverageDiagnostic)} legacyPromotionCoveragePassDiagnosticOnly=${promotionCoveragePass} legacyPromotionAllowedByCoverageDiagnosticOnly=${promotionAllowedByCoverage} decisionUsesSafeOfficialOnlyDiagnosticOnly=${decisionUsesSafeOfficialOnly} useAlternativeForLivePromotion=false alternativeReason=SAFE_OFFICIAL_VERIFIED_COVERAGE_SELECTED executionImpact=NONE`,
    ...
    `PromotionCoveragePolicyDiagnosticOnly: selectedMetricDiagnosticOnly=${selectedMetric} numerator=${selectedNumerator} denominator=${selectedDenominator} requiredDiagnosticOnly=${pct(policy?.requiredVerifiedCoverage ?? 80)} selectedCoverageValueDiagnosticOnly=${pct(selectedCoverageValue)} legacyCoveragePromotionAllowedDiagnosticOnly=${coveragePromotionAllowed} legacyPromotionAllowedDiagnosticOnly=${effectivePromotionAllowed} reason=${promotionReason} alternativeInternalGroupedCoverage=${pct(verifiedCoverageByInternalGrouped)} officialTargetVerifiedCoverageDiagnostic=${pct(officialTargetVerifiedCoverageDiagnostic)} decisionUsesSafeOfficialOnlyDiagnosticOnly=${decisionUsesSafeOfficialOnly} executionImpact=NONE`,
...
export function buildSectorEnergyMasterReportAdr0488(
  ...
  const decisionCoverageRatio = Math.max(after, safeOfficialVerifiedCoverage / 100);
  ...
  const rawStatus: SectorEnergyMasterStatusAdr0488 =
    !evidencePresent ? 'DATA_UNAVAILABLE'
      : decisionCoverageRatio <= 0 ? 'DATA_UNAVAILABLE'
        : sectorDataStale || decisionCoverageRatio < 0.4 || (safeOfficialVerifiedCoverage < 80 && !mapping.symmetryPassed) ? 'DEGRADED'
  ...
  const topGaps = sectorTopGaps({ status, coveragePct: roundPct(decisionCoverageRatio), verifiedIndexCodeCoverage: safeOfficialVerifiedCoverage, mapping, fallbackUsed });
  // ADR-0488: unsafe theme aliases are excluded from official promotion, so their presence is not a decision blocker.
  const leadershipBlockReason: SectorEnergyMasterSupplyLineReportAdr0488['leadershipBlockReason'] =
    safeOfficialVerifiedCoverage < 80 ? 'VERIFIED_INDEX_CODE_COVERAGE_LOW'
      : safeOfficialVerifiedCoverage < 80 && !mapping.symmetryPassed ? 'SYMMETRY_VALIDATION_FAILED'
  ...
  const selectedSectorEnergySourceTier = selectSectorEnergySourceTier({
    ...
    verifiedIndexCodeCoverage: safeOfficialVerifiedCoverage,
  ...
  diagnostics.push(`ADR-0495 seed coverage verified=${adr0495Coverage.coverageVerified}% partial=${adr0495Coverage.coveragePartial}% unknown=${adr0495Coverage.coverageUnknown}%; guardrails executionImpact=NONE liveExecutionAllowed=false legacySectorBoostAllowedDiagnosticOnly=false legacyStrongBuyAllowedDiagnosticOnly=false rawPayloadPersistenceAllowed=false.`);
  diagnostics.push(`SectorEnergy coverage split officialIndexCoverage=${officialIndexCoverage}% verifiedIndexCodeCoverage=${verifiedIndexCodeCoverage}% internalGroupedSnapshotCoverage=${internalGroupedSnapshotCoverage}% internalProxyCoverageAlias=${internalProxyCoverage}% stockDailyFallbackCoverage=${stockDailyFallbackCoverage}%; legacySelectedSourceTierDiagnosticOnly=${selectedSectorEnergySourceTier}; legacyLeadershipConfidenceDiagnosticOnly=${leadershipConfidence}; legacyPromotionAllowedDiagnosticOnly=${promotionAllowed}; leadershipBlockReason=${leadershipBlockReason}.`);
  ...
  diagnostics.push(`Official index master recovery status=${officialIndexMasterRecovery.status}; sourceOfTruth=${officialIndexMasterRecovery.sourceOfTruth}; legacyPromotionAllowedDiagnosticOnly=${promotionAllowed}; executionImpact=NONE.`);
...
function formatSectorEnergyPolicyViewsAdr0488(
  sector: SectorEnergyMasterSupplyLineReportAdr0488,
  canonical: SectorEnergyCanonicalState,
): string[] {
  ...
  return [
    ...
    `legacyPromotionViewDiagnosticOnly: legacyDataQualityDiagnosticOnly=${locked.legacyDataQualityDiagnosticOnly} legacyPromotionAllowedDiagnosticOnly=${locked.legacyPromotionAllowedDiagnosticOnly} legacySectorBoostAllowedDiagnosticOnly=${locked.legacySectorBoostAllowedDiagnosticOnly} legacyStrongBuyAllowedDiagnosticOnly=${locked.legacyStrongBuyAllowedDiagnosticOnly} legacyLeadershipConfidenceDiagnosticOnly=${locked.legacyLeadershipConfidenceDiagnosticOnly} canonicalLocked=${locked.canonicalLocked}`,
  ];
}
...
export function formatSectorEnergySupplyUnknownCompactAdr0488(report: SectorEnergyAndSupplyUnknownPolicyReportAdr0488): string {
  ...
  return [
    ...
    `ADR-0488 SectorEnergyMaster DiagnosticOnly: ${sector.status} | legacySelectedSourceTierDiagnosticOnly=${sector.selectedSectorEnergySourceTier} | coverageBefore=${pct(sector.indexCodeCoverageBefore)} | coverageAfter=${pct(sector.indexCodeCoverageAfter)} | officialIndexCoverage=${pct(sector.officialIndexCoverage)} | verifiedIndexCodeCoverage=${pct(sector.verifiedIndexCodeCoverage)} | internalGroupedSnapshotCoverage=${pct(sector.internalGroupedSnapshotCoverage)} | internalGroupedValidSectorCount=${sector.internalGroupedValidSectorCount ?? 0}/${sector.internalGroupedExpectedSectorCount ?? 0} | internalProxyCoverage=${pct(sector.internalProxyCoverage)} | stockDailyFallbackCoverage=${pct(sector.stockDailyFallbackCoverage)} | missing=${sector.mappingDiagnostics.missingIndexCodeCount} | legacyLeadershipConfidenceDiagnosticOnly=${sector.leadershipConfidence} | legacyPromotionAllowedDiagnosticOnly=${sector.promotionAllowed} | reason=${sector.leadershipBlockReason} | legacySectorBoostAllowedDiagnosticOnly=${sector.sectorBoostAllowed} | legacyStrongBuyAllowedDiagnosticOnly=${sector.strongBuyAllowed} | shadowLeadershipAllowed=${canonical.shadowLeadershipAllowed} | counterfactualAllowed=${canonical.counterfactualAllowed} | impact=${sector.executionImpact}`,
    `SectorEnergy Decision DiagnosticOnly: legacyDataQualityDiagnosticOnly=${sector.leadershipConfidence === 'VERIFIED' ? 'VERIFIED' : sector.status} | selectedPromotionMetricDiagnosticOnly=${sector.selectedPromotionMetric ?? 'SAFE_OFFICIAL_VERIFIED_COVERAGE'} | safeOfficialVerifiedCoverageDiagnosticOnly=${pct(sector.safeOfficialVerifiedCoverage ?? sector.verifiedIndexCodeCoverage)} | requiredPromotionCoverageDiagnosticOnly=${pct(sector.requiredPromotionCoverage ?? 80)} | legacyPromotionCoveragePassDiagnosticOnly=${sector.promotionCoveragePass === true} | legacyPromotionAllowedDiagnosticOnly=${sector.promotionAllowed} | legacyLeadershipConfidenceDiagnosticOnly=${sector.leadershipConfidence} | legacySectorBoostAllowedDiagnosticOnly=${sector.sectorBoostAllowed} | legacyStrongBuyAllowedDiagnosticOnly=${sector.strongBuyAllowed} | decisionUsesSafeOfficialOnlyDiagnosticOnly=${sector.decisionUsesSafeOfficialOnly === true} | unsafeExcluded=${sector.unsafeExcludedNames?.join(',') || 'NONE'} | officialTargetVerifiedCoverageDiagnostic=${pct(sector.officialTargetVerifiedCoverageDiagnostic ?? sector.verifiedIndexCodeCoverage)} | officialTargetCoverageUsedForDecision=false | executionImpact=${sector.executionImpact}`,
    ...formatSectorEnergyPolicyViewsAdr0488(sector, canonical),
    `ADR-0488 OfficialIndexMasterRecovery DiagnosticOnly: ${official.status} | sourceOfTruth=${official.sourceOfTruth} | legacySelectedSourceTierDiagnosticOnly=${official.selectedSectorEnergySourceTier} | officialIndexCoverage=${pct(official.officialIndexCoverage)} | verifiedIndexCodeCoverage=${pct(official.verifiedIndexCodeCoverage)} | requiredVerifiedCoverage=${pct(official.requiredVerifiedCoveragePct)} | legacyPromotionAllowedDiagnosticOnly=${official.promotionAllowed} | legacySectorBoostAllowedDiagnosticOnly=${official.sectorBoostAllowed} | legacyStrongBuyAllowedDiagnosticOnly=${official.strongBuyAllowed} | shadowLeadershipAllowed=${canonical.shadowLeadershipAllowed} | impact=${official.executionImpact}`,
    ...(master ? [
...
export function formatSectorEnergySupplyUnknownDetailAdr0488(report: SectorEnergyAndSupplyUnknownPolicyReportAdr0488): string {
  ...
  return [
    ...
    `- SectorEnergy Decision DiagnosticOnly: legacyDataQualityDiagnosticOnly=${sector.leadershipConfidence === 'VERIFIED' ? 'VERIFIED' : sector.status} selectedPromotionMetricDiagnosticOnly=${sector.selectedPromotionMetric ?? 'SAFE_OFFICIAL_VERIFIED_COVERAGE'} safeOfficialVerifiedCoverageDiagnosticOnly=${pct(sector.safeOfficialVerifiedCoverage ?? sector.verifiedIndexCodeCoverage)} requiredPromotionCoverageDiagnosticOnly=${pct(sector.requiredPromotionCoverage ?? 80)} legacyPromotionCoveragePassDiagnosticOnly=${sector.promotionCoveragePass === true} legacyPromotionAllowedDiagnosticOnly=${sector.promotionAllowed} legacyLeadershipConfidenceDiagnosticOnly=${sector.leadershipConfidence} legacySectorBoostAllowedDiagnosticOnly=${sector.sectorBoostAllowed} legacyStrongBuyAllowedDiagnosticOnly=${sector.strongBuyAllowed} shadowLeadershipAllowed=${canonical.shadowLeadershipAllowed} counterfactualAllowed=${canonical.counterfactualAllowed} decisionUsesSafeOfficialOnlyDiagnosticOnly=${sector.decisionUsesSafeOfficialOnly === true} unsafeExcluded=${sector.unsafeExcludedNames?.join(',') || 'NONE'} officialTargetVerifiedCoverageDiagnostic=${pct(sector.officialTargetVerifiedCoverageDiagnostic ?? sector.verifiedIndexCodeCoverage)} officialTargetCoverageUsedForDecision=false executionImpact=${sector.executionImpact}`,
    ...formatSectorEnergyPolicyViewsAdr0488(sector, canonical).map((line) => `- ${line}`),
    ...
    `- fallbackUsed=${sector.fallbackUsed} legacySelectedSourceTierDiagnosticOnly=${sector.selectedSectorEnergySourceTier} legacyLeadershipConfidenceDiagnosticOnly=${sector.leadershipConfidence} legacyPromotionAllowedDiagnosticOnly=${sector.promotionAllowed} legacySectorBoostAllowedDiagnosticOnly=${sector.sectorBoostAllowed} legacyStrongBuyAllowedDiagnosticOnly=${sector.strongBuyAllowed} shadowLeadershipAllowed=${canonical.shadowLeadershipAllowed} counterfactualAllowed=${canonical.counterfactualAllowed}`,
    `- officialIndexMasterRecovery=${official.status} sourceOfTruth=${official.sourceOfTruth} legacySelectedSourceTierDiagnosticOnly=${official.selectedSectorEnergySourceTier} official=${pct(official.officialIndexCoverage)} verified=${pct(official.verifiedIndexCodeCoverage)} internalGroupedSnapshot=${pct(official.internalGroupedSnapshotCoverage)} internalGroupedValidSectorCount=${official.internalGroupedValidSectorCount ?? 0}/${official.internalGroupedExpectedSectorCount ?? 0} internalProxy=${pct(official.internalProxyCoverage)} stockDailyFallback=${pct(official.stockDailyFallbackCoverage)} legacyPromotionAllowedDiagnosticOnly=${official.promotionAllowed} nextAction=${official.nextAction}`,

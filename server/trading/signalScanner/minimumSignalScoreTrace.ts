...
/**
 * ADR-0611 — SECTOR_RELATIVE_STRENGTH 컴포넌트 재활성 스위치 (default ON since ADR-0645).
 ...
 * ADR-0645 flip — ADR-0644(safe-lever 3종 flip)가 남긴 마지막 점수개선 lever 를 default ON 으로 승격.
 * opt-OUT 정확 비교: `GATE1_SECTOR_RS_COMPONENT_ENABLED=false` 로 즉시 byte-equivalent 롤백
 * (maxScore 0·weightedScore 0). 미설정/임의값 → ON, 정확히 `'false'` 만 OFF (ADR-0157 거울). requiredScore 무변경.
 */

// @responsibility ADR-0484 NAVER Investor Trend Collector wiring tests.
import fs from 'node:fs';
import path from 'node:path';
import { describe, expect, it } from 'vitest';
import {
  buildNaverInvestorTrendCollectorResultAdr0484,
  formatNaverInvestorTrendCompactAdr0484,
  getNaverInvestorTrendDetailRegistryEntryAdr0484,
  safeBuildNaverInvestorTrendCollectorResultAdr0484,
} from './naverInvestorTrendCollectorAdr0484.js';
...
function positive() {
  return buildNaverInvestorTrendCollectorResultAdr0484({
    code: '005930',
    rawPoints: [
...
describe('ADR-0484 NAVER Investor Trend Collector Wiring', () => {
  ...
  it('empty NAVER result returns DATA_UNAVAILABLE/EMPTY and not BEARISH', () => {
    const noRows = buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', rawPoints: [] });
    const empty = buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', rawPoints: [{ date: '2026-05-08' }] });
  ...
  it('parse error, provider error, and non-trading day return UNKNOWN', () => {
    expect(buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', parseError: true }).status).toBe('PARSE_ERROR');
    expect(buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', parseError: true }).signal).toBe('UNKNOWN');
    expect(buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', providerError: true }).status).toBe('PROVIDER_ERROR');
    expect(buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', providerError: true }).signal).toBe('UNKNOWN');
    expect(buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', nonTradingDay: true }).status).toBe('NON_TRADING_DAY');
    expect(buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', nonTradingDay: true }).signal).toBe('UNKNOWN');
  });

  it('stale NAVER data returns STALE and signal UNKNOWN', () => {
    const result = buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', sourceAgeTradingDays: 4, rawPoints: [{ date: '2026-05-01', foreignNetBuy: 10, institutionNetBuy: 20 }] });
  ...
  it('valid positive and negative foreign+institution net-buy produce verified directional signals only with confidence', () => {
    ...
    const bearish = buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', rawPoints: [{ date: '2026-05-08', foreignNetBuy: -10, institutionNetBuy: -20 }] });
  ...
  it('partial and missing data do not force bearish', () => {
    const partial = buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', rawPoints: [{ date: '2026-05-08', foreignNetBuy: -10 }] });
    const missing = buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930' });
  ...
  it('ADR-0477 can select NAVER only in SHADOW_ONLY diagnostics', () => {
    const result = positive();
    const route = buildInvestorFlowProviderRouteResultAdr0477({ code: '005930', naverCollectorResultAdr0484: result, kisTriedForInvestorFlow: true });
  ...
  it('ADR-0473 warmup no longer reports NAVER as NOT_WIRED when ADR-0484 is wired', () => {
    const result = buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930' });
    const report = buildSupplyProviderWarmupReport({ naverInvestorTrendAdr0484: result });
    const text = formatSupplyProviderWarmupCompactLine(report);
    expect(report.naverStatus).not.toBe('NOT_WIRED');
    expect(text).toContain('ADR-0484 NAVER InvestorTrend');
  });
  ...
  it('ADR-0475 dry-run consumes NAVER bullish candidate without live promotion', () => {
    const report = buildGate1PositiveSourceWiringReport({
      ...
      naverInvestorTrendAdr0484: positive(),
      timestamp: '2026-05-09T00:00:00.000Z',
      forDate: '2026-05-09',
      regime: 'TEST',
      marketSession: 'CLOSED',
    });
    expect(report?.naverInvestorTrendCandidatesAdr0484?.[0]).toMatchObject({ status: 'VERIFIED', signal: 'BULLISH', executionImpact: 'NONE', liveExecutionAllowed: false, policyPromotionMode: 'SHADOW_ONLY' });
  });

  it('ADR-0476 records sanitized ADR-0484 observation row without raw NAVER payload', () => {
    const result = positive();
    const rows = buildGate1DryRunObservationRows({ forDate: '2026-05-09', naverInvestorTrendAdr0484: result });
    const row = rows.find((item) => item.observationType === 'NAVER_INVESTOR_TREND_COLLECTOR_ADR0484');
  ...
  it('ADR-0480 downgrades/removes INVESTOR_FLOW_PROVIDER_UNWIRED when NOT_WIRED evidence disappears', () => {
    const report = buildOperatorActionQueueAdr0480({ sources: [{ adr: '0484', sectionId: 'naver_investor_trend', code: 'NAVER_INVESTOR_TREND_EMPTY', diagnosticKey: 'ADR-0484 NAVER_INVESTOR_TREND', diagnosticValue: 'DATA_UNAVAILABLE', severity: 'DATA_UNAVAILABLE' }] });
  ...
  it('ADR-0478 compact and ADR-0479 detail registry include ADR-0484 summary', () => {
    const result = positive();
    expect(formatNaverInvestorTrendCompactAdr0484(result)).toContain('ADR-0484 NAVER InvestorTrend: DATA_AVAILABLE');
    const entry = getNaverInvestorTrendDetailRegistryEntryAdr0484(result);
    expect(entry.adrTraceHint).toBe('/adr_trace 0484');
    expect(entry.render()).toContain('semanticNetBuyCandidate');
  });

  it('collector failure is try/catch isolated', () => {
    const result = safeBuildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', rawPoints: [{ date: null as never }] });
  ...
  it('requiredScore remains 70 and gate thresholds/weights/Kelly remain unchanged by ADR-0484', () => {
    const ledgerSrc = fs.readFileSync(path.resolve('server/trading/signalScanner/gate1DryRunObservationLedgerAdr0476.ts'), 'utf-8');
    const collectorSrc = fs.readFileSync(path.resolve('server/trading/signalScanner/naverInvestorTrendCollectorAdr0484.ts'), 'utf-8');
  ...
  it('UNKNOWN is never bullish and provider issue is never bearish', () => {
    const unavailable = buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930' });
    const providerError = buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', providerError: true });
    expect(unavailable.signal).toBe('UNKNOWN');
    expect(providerError.signal).not.toBe('BEARISH');
  });

  it('ADR-0484 policy invariants are always NONE/false/SHADOW_ONLY/operator approval', () => {
    for (const result of [positive(), buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930' }), buildNaverInvestorTrendCollectorResultAdr0484({ code: '005930', parseError: true })]) {
      expect(result.executionImpact).toBe('NONE');
      expect(result.liveExecutionAllowed).toBe(false);

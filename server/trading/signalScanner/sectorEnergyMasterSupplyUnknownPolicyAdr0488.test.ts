...
describe('ADR-0488 SectorEnergy master + supply UNKNOWN policy', () => {
  ...
  it('uses safe-official verified coverage for promotion with officialTarget verified coverage kept diagnostic-only', () => {
    ...
    expect(compact).toContain('safeOfficialPromotionAllowedDiagnosticOnly=');
    ...
    expect(compact).toContain('livePromotionView: status=SAFE_OFFICIAL_ONLY promotionAllowed=false sectorBoostAllowed=false strongBuyAllowed=false');

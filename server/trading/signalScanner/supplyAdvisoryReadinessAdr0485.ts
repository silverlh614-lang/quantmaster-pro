...
export function buildSupplyAdvisoryReadinessEvidenceAdr0485(input: BuildSupplyAdvisoryReadinessEvidenceInputAdr0485 = {}): SupplyAdvisoryReadinessEvidenceAdr0485 {
  try {
    ...
    const probeRows = input.investorFlowSampleAcquisitionAdr0489 ? 1 : (input.gate1DryRunObservationRowsAdr0476?.filter((row) => row.observationType === 'INVESTOR_FLOW_SAMPLE_ACQUISITION_ADR0489').length ?? 0);
    if (snapshots.length === 0 && !(input.gate1DryRunObservationRowsAdr0476?.length) && probeRows === 0) return emptyEvidence(window);

    const rows = Math.max(snapshots.length, input.gate1DryRunObservationRowsAdr0476?.length ?? 0) + probeRows;
    ...
    const semanticAvailable = snapshots.filter((snapshot) => snapshot.semanticNetBuy.sampleAvailableCount > 0 || snapshot.semanticNetBuy.selectedSampleCount > 0).length + (input.investorFlowSampleAcquisitionAdr0489?.sampleAcquired ? 1 : 0);

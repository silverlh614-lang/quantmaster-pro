...
export const GATE1_POSITIVE_COMPONENT_WEIGHTS: Gate1PositiveComponentWeight[] = [
  { code: 'PRICE_MOMENTUM', enabled: true, maxScore: 15, weight: 15, requiredSource: 'Gate1 momentum/price cache', fallbackPolicy: 'EXCLUDE_FROM_DENOMINATOR', contributesTo: 'SIGNAL_ELIGIBILITY' },
  { code: 'VOLUME_LIQUIDITY', enabled: true, maxScore: 10, weight: 10, requiredSource: 'Gate1 volume/turnover cache', fallbackPolicy: 'EXCLUDE_FROM_DENOMINATOR', contributesTo: 'SIGNAL_ELIGIBILITY' },
  { code: 'TECHNICAL_TREND', enabled: true, maxScore: 15, weight: 15, requiredSource: 'technical indicator cache', fallbackPolicy: 'EXCLUDE_FROM_DENOMINATOR', contributesTo: 'SIGNAL_ELIGIBILITY' },
  { code: 'RELATIVE_STRENGTH', enabled: true, maxScore: 15, weight: 15, requiredSource: '20d/60d price history or watchlist proxy', fallbackPolicy: 'DIAGNOSTIC_ONLY', contributesTo: 'SIGNAL_ELIGIBILITY' },
  { code: 'WATCHLIST_UPSTREAM_SCORE', enabled: true, maxScore: 15, weight: 15, requiredSource: 'watchlist rank/score/reason', fallbackPolicy: 'DIAGNOSTIC_ONLY', contributesTo: 'SIGNAL_ELIGIBILITY' },
  { code: 'WATCHLIST_PRIORITY', enabled: true, maxScore: 5, weight: 5, requiredSource: 'watchlist priority', fallbackPolicy: 'NEUTRAL_IF_MISSING', contributesTo: 'SIGNAL_ELIGIBILITY' },
  ...
  { code: 'VCP_OR_VOLATILITY_COMPRESSION', enabled: true, maxScore: 5, weight: 5, requiredSource: 'OHLCV compression detector', fallbackPolicy: 'DIAGNOSTIC_ONLY', contributesTo: 'SIGNAL_ELIGIBILITY' },
  { code: 'SECTOR_RELATIVE_STRENGTH', enabled: true, maxScore: 5, weight: 5, requiredSource: 'sector relative return', fallbackPolicy: 'EXCLUDE_FROM_DENOMINATOR', contributesTo: 'SIGNAL_ELIGIBILITY' },
  { code: 'MARKET_REGIME_SUPPORT', enabled: true, maxScore: 5, weight: 5, requiredSource: 'macro regime', fallbackPolicy: 'NEUTRAL_IF_MISSING', contributesTo: 'SIGNAL_ELIGIBILITY' },

// @responsibility ADR-0483 supply source freshness dual-clock tests.
import fs from 'node:fs';
import path from 'node:path';
import { describe, expect, it } from 'vitest';
import {
  buildSupplySourceFreshnessItemAdr0483,
  buildSupplySourceFreshnessReportAdr0483,
  deriveSupplyRefreshJobStatusAdr0483,
  formatSupplySourceFreshnessCompactAdr0483,
  getSupplySourceFreshnessDetailRegistryEntryAdr0483,
  safeBuildSupplySourceFreshnessReportAdr0483,
...
function staleReport() {
  return buildSupplySourceFreshnessReportAdr0483({ generatedAt: NOW, inputs: [
    staleInput(),
...
function semanticReportWithFreshness(report = staleReport()) {
  return buildSemanticNetBuyNormalizationReportAdr0482({
    code: '005930',
    generatedAt: NOW,
    supplySourceFreshnessAdr0483: report,
    inputs: [{
      code: '005930',
      provider: 'NAVER',
...
describe('ADR-0483 Supply Source Freshness Dual Clock', () => {
  it('cache fresh + source stale keeps separate clocks', () => {
    const item = buildSupplySourceFreshnessItemAdr0483(staleInput());
  ...
  it('source age thresholds produce FRESH, DEGRADED severity, and STALE', () => {
    const fresh = buildSupplySourceFreshnessItemAdr0483(staleInput({ sourceDate: '2026-05-08' }));
    const degraded = buildSupplySourceFreshnessItemAdr0483(staleInput({ sourceDate: '2026-05-06' }));
    const stale = buildSupplySourceFreshnessItemAdr0483(staleInput({ sourceDate: '2026-05-01' }));
  ...
  it('null sourceDate returns MISSING or DATA_UNAVAILABLE', () => {
    expect(buildSupplySourceFreshnessItemAdr0483(staleInput({ sourceDate: null })).sourceState).toBe('MISSING');
    expect(buildSupplySourceFreshnessItemAdr0483(staleInput({ sourceDate: null, sourceStatus: 'DATA_UNAVAILABLE' })).sourceState).toBe('DATA_UNAVAILABLE');
  });

  it('NON_TRADING_DAY, PROVIDER_ERROR, and stale source are provider issues not market signals', () => {
    const nonTrading = buildSupplySourceFreshnessItemAdr0483(staleInput({ isTradingDay: false, sourceStatus: 'NON_TRADING_DAY' }));
    const providerError = buildSupplySourceFreshnessItemAdr0483(staleInput({ sourceStatus: 'PROVIDER_ERROR' }));
    const stale = buildSupplySourceFreshnessItemAdr0483(staleInput());
  ...
  it('refresh status rules handle fresh, stale, non-trading, and dry-run', () => {
    expect(deriveSupplyRefreshJobStatusAdr0483({ sourceState: 'FRESH', severity: 'OK' })).toBe('NOT_REQUIRED');
    expect(deriveSupplyRefreshJobStatusAdr0483({ sourceState: 'STALE', severity: 'STALE' })).toBe('RECOMMENDED');
    expect(deriveSupplyRefreshJobStatusAdr0483({ sourceState: 'NON_TRADING_DAY', severity: 'INFO' })).toBe('SKIPPED_NON_TRADING_DAY');
    expect(deriveSupplyRefreshJobStatusAdr0483({ sourceState: 'STALE', severity: 'STALE', dryRunRefresh: true })).toBe('DRY_RUN');
  });

  it('ADR-0473 warmup includes ADR-0483 freshness summary', () => {
    const report = buildSupplyProviderWarmupReport({ supplySourceFreshnessAdr0483: staleReport() });
    const text = formatSupplyProviderWarmupCompactLine(report);
    expect(text).toContain('ADR-0483 SupplyFreshness: STALE');
    expect(text).toContain('impact=NONE');
  });

  it('ADR-0477 degrades confidence when selected sample source is stale', () => {
    const freshness = buildSupplySourceFreshnessReportAdr0483({ generatedAt: NOW, inputs: [staleInput({ sourceId: 'NAVER_INVESTOR_TREND', provider: 'NAVER', sourceDate: '2026-05-01' })] });
    const semantic = buildSemanticNetBuyNormalizationReportAdr0482({ code: '005930', generatedAt: NOW, inputs: [{ code: '005930', provider: 'NAVER', sourceDate: '2026-05-08', rawForeignNetBuy: 10, rawInstitutionNetBuy: 20, unit: 'KRW' }] });
    const route = buildInvestorFlowProviderRouteResultAdr0477({ code: '005930', semanticNetBuyNormalizationAdr0482: semantic, supplySourceFreshnessAdr0483: freshness });
    expect(route.status).toBe('STALE');
    expect(route.semanticNetBuy?.confidence).toBe('LOW');
    expect(route.signal).toBe('UNKNOWN');
  });

  it('ADR-0482 attaches sourceAgeTradingDays from ADR-0483 freshness', () => {
    const report = semanticReportWithFreshness(buildSupplySourceFreshnessReportAdr0483({ generatedAt: NOW, inputs: [staleInput({ sourceId: 'NAVER_INVESTOR_TREND', provider: 'NAVER', sourceDate: '2026-05-01' })] }));
  ...
  it('ADR-0476 records sanitized freshness observation row', () => {
    const rows = buildGate1DryRunObservationRows({ forDate: '2026-05-09', supplySourceFreshnessAdr0483: staleReport() });
    const row = rows.find((item) => item.observationType === 'SUPPLY_SOURCE_FRESHNESS_ADR0483');
  ...
  it('ADR-0480 creates stale refresh and cache/source mismatch actions while fresh evidence stays quiet', () => {
    const refresh = buildOperatorActionQueueAdr0480({ sources: [{ adr: '0483', sectionId: 'supply_source_freshness', code: 'ADR-0483', diagnosticKey: 'ADR-0483 SupplyFreshness', diagnosticValue: 'STALE refresh recommended', severity: 'DEGRADED' }] });
    const mismatch = buildOperatorActionQueueAdr0480({ sources: [{ adr: '0483', sectionId: 'supply_source_freshness', code: 'ADR-0483', diagnosticKey: 'ADR-0483 SupplyFreshness', diagnosticValue: 'cacheState=FRESH sourceState=STALE', severity: 'DEGRADED' }] });
    const fresh = buildOperatorActionQueueAdr0480({ sources: [{ adr: '0483', sectionId: 'supply_source_freshness', code: 'ADR-0483', diagnosticKey: 'ADR-0483 SupplyFreshness', diagnosticValue: 'OK oldest=1d', severity: 'INFO' }] });
  ...
  it('ADR-0478 compact output and ADR-0479 detail registry expose ADR-0483 trace', () => {
    const report = staleReport();
    expect(formatSupplySourceFreshnessCompactAdr0483(report)).toContain('ADR-0483 SupplyFreshness: STALE');
    const entry = getSupplySourceFreshnessDetailRegistryEntryAdr0483(report);
    expect(entry.adrTraceHint).toBe('/adr_trace 0483');
    expect(entry.render()).toContain('cache=FRESH');
  });

  it('does not import KIS order modules or live execution modules', () => {
    const src = fs.readFileSync(path.resolve('server/trading/signalScanner/supplySourceFreshnessAdr0483.ts'), 'utf-8');
  ...
  it('policy invariants are always NONE/false/SHADOW_ONLY/operator approval and failures are isolated', () => {
    const report = staleReport();
    const failed = safeBuildSupplySourceFreshnessReportAdr0483({ inputs: [{ ...staleInput(), sourceDate: Symbol('bad') as never }] });

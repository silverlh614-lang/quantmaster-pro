...
function buildFreshDataSchedulerRowsAdr0492(input: Gate1DryRunObservationBuildInput): Gate1DryRunObservationRow[] {
  const report = input.freshDataSchedulerAdr0492;
  if (!report) return [];
  return [withOptionalScoreFields({
    ...buildFreshDataSchedulerObservationRowAdr0492(report),
    observationType: 'FRESH_DATA_SCHEDULER_ADR0492' as const,
  } as Omit<Gate1DryRunObservationRow, 'id'> & { id?: string })];
}
...
export function buildGate1DryRunObservationRows(input: Gate1DryRunObservationBuildInput): Gate1DryRunObservationRow[] {
  ...
  const rows = [
    ...
    ...buildFreshDataSchedulerRowsAdr0492(input),
...
export function formatGate1DryRunObservationSummary(
  ...
  return [
    ...
    sourceLine('ADR_0492_FRESH_DATA_SCHEDULER'),

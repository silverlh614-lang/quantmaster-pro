...
function escapeHtmlDiagnosticSample(value: unknown): string {
  return String(value)
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function formatReasonDistribution(
...
function formatSampleList(samples: string[]): string {
  return samples.length > 0
    ? samples
        .map((sample, index) => `${index + 1}. "${escapeHtmlDiagnosticSample(sample)}"`)
        .join("\n  ")
    : "none";
}

function formatStockProgramFieldKeysTop(
  keys: string[],
...
function nextActionForProgramReason(
  reason: string,

...
describe('addBuyBlockGate', () => {
  it('shadows 빈 배열 → pass=true', () => {
    const r = addBuyBlockGate(makeMockCtx({ shadows: [] }));
    expect(r.pass).toBe(true);
  });

  it('동일 종목 shadow + addBuyBlocked=true → pass=false', () => {
    const stock = makeMockStock({ code: '005930' });
    const shadow = makeMockShadow({ stockCode: '005930', addBuyBlocked: true });
    const r = addBuyBlockGate(makeMockCtx({ stock, shadows: [shadow] }));
  ...
  it('addBuyBlocked=false → pass=true (플래그 미설정 시나리오)', () => {
    const shadow = makeMockShadow({ stockCode: '005930', addBuyBlocked: false });
    const r = addBuyBlockGate(makeMockCtx({ shadows: [shadow] }));
    expect(r.pass).toBe(true);
  });

  it('다른 종목 shadow 가 차단 플래그여도 본 종목은 통과', () => {
    const stock = makeMockStock({ code: '005930' });
    const otherShadow = makeMockShadow({ stockCode: '000660', addBuyBlocked: true });
    const r = addBuyBlockGate(makeMockCtx({ stock, shadows: [otherShadow] }));
    expect(r.pass).toBe(true);
  });

  it('동일 종목 다중 shadows 중 하나라도 addBuyBlocked → pass=false', () => {
    const stock = makeMockStock({ code: '005930' });
    const s1 = makeMockShadow({ stockCode: '005930', addBuyBlocked: false });
    const s2 = makeMockShadow({ stockCode: '005930', addBuyBlocked: true });
    const r = addBuyBlockGate(makeMockCtx({ stock, shadows: [s1, s2] }));
    expect(r.pass).toBe(false);
  });
});

...
describe('rrrGate', () => {
  it('정상 RRR (>임계) → pass=true (entry=100, target=120, stop=90 → RRR=2.0)', () => {
    const r = rrrGate(makeMockCtx({ stock: makeMockStock({ entryPrice: 100, targetPrice: 120, stopLoss: 90 }) }));
    expect(r.pass).toBe(true);
  });

  it('RRR 미달 → pass=false + 부수효과 명시', () => {
    ...
    const r = rrrGate(makeMockCtx({ stock }));
  ...
  it('logMessage 에 임계값 + 종목명 + 코드 + RRR 값 포함', () => {
  ...
  it('차단 시 stageLog.value 에 RRR 수치 + 임계 둘 다 노출', () => {
    const stock = makeMockStock({ entryPrice: 100, targetPrice: 105, stopLoss: 95 });
    const r = rrrGate(makeMockCtx({ stock }));
    if (!r.pass) {
      // 'FAIL(0.50 < 1.5)' 같은 형식

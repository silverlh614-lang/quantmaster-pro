...
describe('resolveKospiTriggerFreshness (ADR-0590 D1)', () => {
  // 케이스 1: trade-date=오늘 → freshness 보존, intraday 트리거 active 유지.
  ...
  // 케이스 2: trade-date=어제(폭락) + age 짧음(FRESH) → STALE 강등(결함 A 회귀 방지).
  it('downgrades FRESH age to STALE when trade-date bar is yesterday (defect A)', () => {
  ...
  it('downgrades SOFT_STALE age to STALE when trade-date bar is yesterday', () => {
    const r = resolveKospiTriggerFreshness({ tradeDate: YESTERDAY_KEY, ageFreshness: 'SOFT_STALE', now: NOW });
    expect(r.freshness).toBe('STALE');
    expect(r.downgraded).toBe(true);
  });

  // 케이스 3: trade-date 부재/파싱 실패 → legacy age-only 폴백(보수).
  ...
  // 케이스 4: close-shock 계열(POST_CLOSE/EOD)은 trade-date 어제여도 강등 분리(EOD 경로 보존).
  it('does NOT downgrade POST_CLOSE_VALID even when trade-date is yesterday (close-shock separated)', () => {
  ...
  it('does NOT downgrade EOD_SNAPSHOT_VALID when trade-date is yesterday', () => {
  ...
  it('keeps already-stale freshness unchanged when trade-date is yesterday (no further damage)', () => {
  ...
  // 거래일이 아닌 now(주말) → 봉 거래일 게이트 적용 불가 → legacy 폴백(보수).

 ...
 *   - flag OFF(KIS_OHLCV_PRIMARY_ENABLED!=='true'): 기존 `fetchCloses(sym, range)` Yahoo funnel 그대로
 *     위임·반환 → byte-equal(호출수·심볼후보순서·반환형 number[]|null 동일, 청산결정 불변).
 *   - flag ON: KIS 일봉(FHKST03010100, fetchKisDailyCandles) 종가배열(과거→최신, Yahoo 동형) 1차.
 *     KIS 불가(빈 배열/throw) 시 Yahoo funnel 로 graceful fallback(불변식 #6 — provider 장애≠약세).
 *
 * 신규 종가 계산·청산 산식 0줄 — 데이터 소스 경로만 분기. 기본 OFF(byte-equivalent).
 * 활성화(flag ON=KIS)는 shadow A/B 검증 후 별도 PR(이번 아님).
 */

import { fetchCloses } from '../../marketDataRefresh.js';
import { fetchKisDailyCandles } from '../../../screener/kisChartDataFetcher.js';

/** ENV 글로벌 토글(default OFF) — R1(ADR-0547) 과 동일 flag 재사용(동일 도메인: 국내 OHLCV 종가). */

...
describe('applyFomcRelaxation v3 — DAY 만 우호 환경 완화 적용', () => {
  describe('DAY 외 phase — 완화 무관 (정상 운용 또는 부스트 그대로)', () => {
...
describe('generateFomcIcs — v3 정책 반영', () => {
  it('ICS DESCRIPTION 이 "D-day(발표 당일) 신규 진입 자동 차단" 으로 갱신', () => {
    const ics = generateFomcIcs();
    expect(ics).toContain('D-day(발표 당일) 신규 진입 자동 차단');
    expect(ics).toContain('우호 환경 시 D-day 도 보수적 진입');
    expect(ics).not.toContain('D-3부터 신규 진입 자동 차단');
    expect(ics).not.toContain('D-1 부터 신규 진입 자동 차단');
  });

  it('ICS VALARM D-1 경보 메시지가 "정상 운용" 안내로 갱신 (v3)', () => {
    const ics = generateFomcIcs();
    expect(ics).toContain('TRIGGER:-P1DT0H0M0S');
    expect(ics).toContain('D-day 만 신규 진입 자동 차단');
    expect(ics).toContain('D-1 은 정상 운용');

...
export function resolveRegimeSnapshot(options: ResolveRegimeSnapshotOptions = {}): ResolvedRegimeSnapshot {
  ...
  try {
    ...
    const baseSnapshot: RegimeSnapshot = {
      ...
      marketSignal: false,
      macroFreshness: marketState.macroState.freshness,
      macroAgeSec: marketState.macroState.ageSec,
      macroLastRefreshAttemptAt: marketState.macroState.lastRefreshAttemptAt,
      macroRefreshJobLastRunAt: marketState.macroState.refreshJobLastRunAt,
      regimeReleaseAllowed: marketState.macroState.freshness !== 'HARD_STALE' && marketState.macroState.freshness !== 'MISSING',
      regimeReleaseBlockedReason: marketState.macroState.freshness === 'HARD_STALE' ? 'MACRO_HARD_STALE' : marketState.macroState.freshness === 'MISSING' ? 'MACRO_MISSING' : 'NONE',
      executionImpact: marketState.macroState.executionImpact,
      staleSources: marketState.staleSources,
      rawMhsLabel: marketState.mhsLabel,
      rawBiasLabel: marketState.biasLabel,
      selectedDisplaySource: riskOverride === 'NONE' ? 'effectiveRegime' : 'riskOverride',
      correctionApplied: displayRegime !== (marketState.displayTitle || effectiveRegime),
      userVisibleSafe: riskOverride === 'R6_DEFENSE' ? displayRegime === 'R6_DEFENSE' : true,
      conflicts: [],
    };

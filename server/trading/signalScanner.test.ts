...
describe('[C1] activeSwingCount — INTRADAY·PRE_BREAKOUT 포지션 제외 검증', () => {
  ...
  // 회귀 방지: 사전 점검(activeSwingCount)과 루프 내 재점검(currentActive)은
  // 반드시 동일한 제외 기준(INTRADAY + PRE_BREAKOUT)을 써야 한다.
  // 기준이 어긋나면 "보유 슬롯이 남았는데 매수가 일어나지 않는" 무성 실패가 난다.
  it('currentActive 루프 필터는 activeSwingCount 사전 필터와 동일해야 한다', () => {
    const shadows = [
      { status: 'ACTIVE', watchlistSource: 'PRE_MARKET' },     // 스윙 — 포함
      { status: 'ACTIVE', watchlistSource: 'PRE_MARKET' },     // 스윙 — 포함
      { status: 'ACTIVE', watchlistSource: 'PRE_MARKET' },     // 스윙 — 포함
    ...
    const isSwingSlot = (s: (typeof shadows)[number]) =>
      isOpenShadowStatus(s.status) &&
      s.watchlistSource !== 'INTRADAY' &&
      s.watchlistSource !== 'PRE_BREAKOUT';

    const activeSwingCount = shadows.filter(isSwingSlot).length;
    const currentActive    = shadows.filter(isSwingSlot).length;

    // 동일 기준: 두 카운터는 항상 같아야 한다 (3개)
    expect(currentActive).toBe(activeSwingCount);
    expect(currentActive).toBe(3);

    // maxPositions=5 일 때 사전 점검은 "여유 2슬롯" 이라 판정 → 루프 점검도 동일해야
    // 1개 매수가 예약(reservedSlots=1)된 뒤 남은 1슬롯이 사용 가능해진다.
    const maxPositions = 5;
    const reservedSlots = 1;
    const totalCommitted = currentActive + reservedSlots;
    expect(totalCommitted).toBeLessThan(maxPositions);
  });
});

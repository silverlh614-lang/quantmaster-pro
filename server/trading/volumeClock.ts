 ...
 * │  11:30 ~ 12:59  점심 구간 — 거래량 저조, Scanner SELL_ONLY 연동              │
...
const BLOCKED_WINDOWS: Array<{ start: number; end: number; label: string }> = [
  ...
  { start: 11 * 60 + 30, end: 12 * 60 + 59, label: '11:30~12:59 점심 구간 — 거래량 저조' },
...
const TIME_ZONES: Array<{ start: number; end: number; bonus: number; label: string }> = [
  ...
  // 11:30~12:59 → BLOCKED_WINDOWS로 이동 (점심 구간 매수 차단, Scanner SELL_ONLY 연동)

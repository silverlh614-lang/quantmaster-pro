...
export function formatLearningPulseMessage(s: ReturnType<typeof collectLearningPulse>): string {
  ...
  const extra = [
    ...
    `Fresh Shadow Inlet: marketSession=${(s.freshShadowInlet as any).marketSession ?? ((s.freshShadowInlet as any).marketOpen === false ? 'CLOSED' : 'OPEN')} / liveBlocker=${s.freshShadowInlet.liveBlocker ?? 'NONE'} / shadowInletStatus=${s.freshShadowInlet.shadowInletStatus ?? s.freshShadowInlet.nextAction} / queuedNextOpenShadowScan=${String((s.freshShadowInlet as any).queuedNextOpenShadowScan ?? false)} / nextShadowScanAt=${(s.freshShadowInlet as any).nextShadowScanAt ?? 'N/A'} / nextShadowScanReason=${(s.freshShadowInlet as any).nextShadowScanReason ?? 'N/A'} / nextOpenShadowScanStatus=${(s.freshShadowInlet as any).nextOpenShadowScanStatus ?? 'N/A'} / shadowScanLifecycleStatus=${(s as any).shadowScanLifecycleStatus ?? 'N/A'} / lastShadowScanStartedAt=${(s.freshShadowInlet as any).lastShadowScanStartedAt ?? 'N/A'} / shadowScanStartedAtSource=${(s.freshShadowInlet as any).shadowScanStartedAtSource ?? 'N/A'} / lastShadowScanCandidateCount=${(s.freshShadowInlet as any).lastShadowScanCandidateCount ?? 0} / lastShadowScanResult=${(s.freshShadowInlet as any).lastShadowScanResult ?? 'N/A'} / counterfactualCreatedToday=${(s as any).counterfactualCreatedToday ?? 0} / evaluatedCandidateCount=${(s as any).evaluatedCandidateCount ?? 0} / r6BlockedButCounterfactualCreatedCount=${(s as any).r6BlockedButCounterfactualCreatedCount ?? 0} / nextOpenShadowScanLogEvents=${JSON.stringify((s.freshShadowInlet as any).nextOpenShadowScanLogEvents ?? [])} / warning=${(s.freshShadowInlet as any).warning ?? 'NONE'} / scanCandidatesToday=${s.freshShadowInlet.scanCandidatesToday} / shadowSignalsToday=${s.freshShadowInlet.shadowSignalsToday} / shadowApprovedToday=${s.freshShadowInlet.shadowApprovedToday} / shadowOrdersCreatedToday=${s.freshShadowInlet.shadowOrdersCreatedToday} / paperFilledToday=${s.freshShadowInlet.paperFilledToday} / counterfactualAllowed=${s.freshShadowInlet.counterfactualAllowed} / executionImpact=${s.freshShadowInlet.executionImpact}`,

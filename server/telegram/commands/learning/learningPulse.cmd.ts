...
const learningPulse: TelegramCommand = { name: '/learning_pulse', aliases: ['/lp'], category: 'LRN', visibility: 'ADMIN', riskLevel: 0, description: '학습 루프 v3 진단 (repair/outcome/open/attribution/suggest/gemini)', async execute({ reply }) { try { await reply(formatLearningPulseMessage(collectLearningPulse())); } catch (e) { await reply(`⚠️ /learning_pulse 실패: ${e instanceof Error ? e.message : String(e)}`); } } };

...
import { collectRegimeLearningBank, collectRegimeLearningConsistency, formatRegimeLearningSummary } from '../../../learning/regimeLearningBank.js';
...
function splitCounterfactualSuggestBlockers(input: {
  ...
  if (!input.historicalView && input.activeRegimeResolvedSampleSize < 20) secondaryBlockers.push('LOW_RESOLVED_REGIME_SAMPLE');
  if (!input.historicalView && input.trueUnknownRatio > unknownThreshold) secondaryBlockers.push('UNKNOWN_RATIO_HIGH');
  if (!input.historicalView && input.recoveredLowConfidenceRegimeRatio > 0) secondaryBlockers.push('LOW_CONFIDENCE_REGIME_BACKFILL');
...
export function collectLearningPulse(now: Date = new Date()) {
  ...
  const blockerSplit = splitCounterfactualSuggestBlockers({
    ...
    historicalView: regimeLearning.activeRegime === 'HISTORICAL_ALL',
  });
...
export function formatLearningPulseMessage(s: ReturnType<typeof collectLearningPulse>): string {
  const historicalView = s.regimeLearning.activeRegime === 'HISTORICAL_ALL';
  const historicalTotals = s.regimeLearning.stats.reduce((total, row) => ({
    samples: total.samples + row.totalSampleSize,
    resolved: total.resolved + row.resolvedSampleSize,
    pending: total.pending + row.pendingCounterfactualCount + row.pendingShadowCount,
  }), { samples: 0, resolved: 0, pending: 0 });
  ...
  const extra = [
    ...
    '6. Historical Regime Learning',
    historicalView
      ? `historyView=ALL / runtimeRegime=RETIRED / total=${historicalTotals.samples} / resolved=${historicalTotals.resolved} / pending=${historicalTotals.pending} / executionImpact=NONE`
      : `historicalRegime=${s.regimeLearning.activeRegime} / status=COLLECTING_PENDING_OUTCOMES / resolved=${s.regimeLearning.activeRegimeResolvedSampleSize}/100 / pending=${s.regimeLearning.activeRegimePendingCounterfactualCount} / severity=INFO`,

...
import { collectCounterfactualStatus, collectLearningCohortSummary } from '../../../learning/learningSampleQuality.js';
import { collectFreshOnlyPromotion, collectFreshShadowStatus } from '../../../shadow/freshShadowLifecycle.js';
...
export function formatLearningPulseMessage(s: ReturnType<typeof collectLearningPulse>): string {
  ...
  return [formatLearningPulseV3(s.v3), `🩺 Learning Pulse v5 (${s.todayKst})`, `🌱 Fresh Shadow: todayFreshCandidates ${s.freshShadow.todayFreshCandidates} / freshOpen ${s.freshShadow.freshShadowOpen} / freshClosed7d ${s.freshShadow.freshShadowClosed7d} / freshSampleSize ${s.freshPromotion.freshSampleSize} / freshExpectancyR ${s.freshPromotion.freshExpectancyR.toFixed(4)} / freshLifecycleBreaks ${s.freshShadow.lifecycleBreaks} / blocker ${s.freshPromotion.blocker}`, `🧪 Counterfactual: candidateCount ${s.counterfactual.candidateCount} / builtCount ${s.counterfactual.builtCount} / labeledCount ${s.counterfactual.labeledCount} / pendingOutcome ${s.counterfactual.pendingOutcome} / blocker ${s.counterfactual.blocker}`, `🚦 Promotion: basis=${s.freshPromotion.basis} / freshSampleSize=${s.freshPromotion.freshSampleSize} / requiredFreshSamples=${s.freshPromotion.requiredFreshSamples} / promotionAllowed=${s.freshPromotion.promotionAllowed} / blockers=${s.freshPromotion.blockers.join(', ') || s.freshPromotion.blocker}`, `👻 Ghost Repair: ghostRepairCount=${s.cohorts.ghostRepairCount} / ghostRepairExpectancyR=${String(s.cohorts.expectancyByCohort.GHOST_REPAIR ?? 'N/A')} / diagnosticOnly=true`, `👻 Ghost Portfolio: OPEN ${s.ghost.open} / 7일 close ${s.ghost.closedRecent7d} / closeRate7d ${(s.ghost.closeRate7d*100).toFixed(1)}% / lastGhostCloseRunCloseRate ${typeof s.v3.rateMetrics.lastGhostCloseRunCloseRate === 'number' ? `${(s.v3.rateMetrics.lastGhostCloseRunCloseRate*100).toFixed(1)}%` : 'N/A'} / lastLearningRepairRunCloseRate ${typeof s.v3.rateMetrics.lastLearningRepairRunCloseRate === 'number' ? `${(s.v3.rateMetrics.lastLearningRepairRunCloseRate*100).toFixed(1)}%` : 'N/A'} / closeRateFormula=${s.v3.rateMetrics.closeRateFormula} / staleOpenCount ${s.ghost.staleOpenCount}`, `📊 Attribution: ${s.attribution7d.count}/${s.attribution7d.target} (7d) / starvationReason ${s.attribution.starvationReason}${s.attribution.previousStarvationReason ? ` / previousStarvationReason ${s.attribution.previousStarvationReason}` : ''}`, `⚖️ Condition Weights: changed ${s.weights.changedFromDefault} / untouched ${s.weights.untouched} / sunset ${s.weights.sunsetCount}`, `🔔 Suggest Metrics: suggestSignals7d ${suggestSignals7d} / diagnosticProposals7d ${s.diagnosticProposals7d} / autoApplied7d 0 / totalDiagnosticEvents7d ${s.diagnosticProposals7d} / blocker ${s.suggest.blocker} / status DIAGNOSTIC_ONLY / legacyTotal7d ${totalSuggest}`, `🧪 Experiment Proposal: active ${s.experimentsActive}`, `🤖 Gemini: 호출 ${s.gemini.callCount}회 / ~${LEARNING_TRADING_DAYS_PER_MONTH}영업일 → 호출률 ${(s.gemini.useRatio*100).toFixed(0)}% / nextScheduledAt ${s.gemini.nextScheduledAt ?? 'N/A'} / recommendationOnly=true`, s.flags.length ? `🚩 진단 플래그: ${s.flags.join(', ')}` : '✅ 모든 채널 정상', s.partialFailure ? '⚠️ 데이터 일부 미확인 — 손상/누락 저장소가 있어 가능한 영역만 표시' : ''].filter(Boolean).join('\n');
}

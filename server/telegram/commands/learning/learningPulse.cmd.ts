...
function splitCounterfactualSuggestBlockers(input: {
  ...
  return { primaryBlocker, secondaryBlockers: [...new Set(secondaryBlockers)].filter((b) => b !== primaryBlocker) };
}
...
export function formatLearningPulseMessage(s: ReturnType<typeof collectLearningPulse>): string {
  ...
  const extra = [
    ...
    `Fresh Shadow Inlet: liveBlocker=${s.freshShadowInlet.liveBlocker ?? 'NONE'} / shadowInletStatus=${s.freshShadowInlet.shadowInletStatus ?? s.freshShadowInlet.nextAction} / queuedNextOpenShadowScan=${String((s.freshShadowInlet as any).queuedNextOpenShadowScan ?? false)} / nextShadowScanAt=${(s.freshShadowInlet as any).nextShadowScanAt ?? 'N/A'} / nextShadowScanReason=${(s.freshShadowInlet as any).nextShadowScanReason ?? 'N/A'} / nextOpenShadowScanStatus=${(s.freshShadowInlet as any).nextOpenShadowScanStatus ?? 'SCHEDULED'} / lastShadowScanStartedAt=${(s.freshShadowInlet as any).lastShadowScanStartedAt ?? 'N/A'} / lastShadowScanCandidateCount=${(s.freshShadowInlet as any).lastShadowScanCandidateCount ?? 0} / lastShadowScanResult=${(s.freshShadowInlet as any).lastShadowScanResult ?? 'N/A'} / scanCandidatesToday=${s.freshShadowInlet.scanCandidatesToday} / shadowSignalsToday=${s.freshShadowInlet.shadowSignalsToday} / shadowApprovedToday=${s.freshShadowInlet.shadowApprovedToday} / shadowOrdersCreatedToday=${s.freshShadowInlet.shadowOrdersCreatedToday} / paperFilledToday=${s.freshShadowInlet.paperFilledToday} / counterfactualAllowed=${s.freshShadowInlet.counterfactualAllowed} / executionImpact=${s.freshShadowInlet.executionImpact}`,
    ...
    `Regime Learning v6: activeRegime=${s.regimeLearning.activeRegime} / rawRegime=${s.regimeLearning.rawRegime} / effectiveRegime=${s.regimeLearning.effectiveRegime} / shadowLearningAllowed=${s.regimeLearning.shadowLearningAllowed} / regimeLearningSampleSize=${s.regimeLearning.regimeLearningSampleSize} / regimeAssignedCount=${s.regimeLearning.regimeAssignedCount} / unknownRegimeCount=${s.regimeLearning.unknownRegimeCount} / recoveredLowConfidenceRegimeCount=${(s.regimeLearning as any).recoveredLowConfidenceRegimeCount ?? 0} / trueUnknownRegimeCount=${(s.regimeLearning as any).trueUnknownRegimeCount ?? s.regimeLearning.unknownRegimeCount} / unknownRatio=${s.regimeLearning.unknownRatio} / trueUnknownRatio=${(s.regimeLearning as any).trueUnknownRatio ?? s.regimeLearning.unknownRatio} / regimeBackfillAttempted=${(s.regimeLearning as any).regimeBackfillAttempted ?? 0} / regimeBackfillRecovered=${(s.regimeLearning as any).regimeBackfillRecovered ?? 0} / regimeBackfillFailed=${(s.regimeLearning as any).regimeBackfillFailed ?? 0} / regimeBackfillWindowMinutes=${(s.regimeLearning as any).regimeBackfillWindowMinutes ?? 60} / regimeBackfillFailureTopReasons=${JSON.stringify((s.regimeLearning as any).regimeBackfillFailureTopReasons ?? [])} / regimeDuplicateCandidates=${(s.regimeLearning as any).regimeDuplicateCandidates ?? 0} / regimeDuplicateSuppressed=${(s.regimeLearning as any).regimeDuplicateSuppressed ?? 0} / regimeDuplicateSourceTop3=${JSON.stringify((s.regimeLearning as any).regimeDuplicateSourceTop3 ?? [])} / regimeDuplicateRootCause=${(s.regimeLearning as any).regimeDuplicateRootCause ?? 'NONE'} / regimeDedupStatus=${(s.regimeLearning as any).regimeDedupStatus ?? 'NONE'} / bestRegimeByExpectancy=${s.regimeLearning.bestRegimeByExpectancy ?? 'N/A'} / worstRegimeByExpectancy=${s.regimeLearning.worstRegimeByExpectancy ?? 'N/A'} / activeRegimeSampleSize=${s.regimeLearning.activeRegimeSampleSize} / activeRegimeTotalSampleSize=${s.regimeLearning.activeRegimeTotalSampleSize} / activeRegimeResolvedSampleSize=${s.regimeLearning.activeRegimeResolvedSampleSize} / activeRegimePendingCounterfactualCount=${s.regimeLearning.activeRegimePendingCounterfactualCount} / activeRegimeAttributableSampleSize=${s.regimeLearning.activeRegimeAttributableSampleSize} / activeRegimeExpectancyR=${activeRegimeExpectancyR} / activeRegimeQualityStatus=${s.regimeLearning.activeRegimeQualityStatus} / activeRegimeWhyNotReliable=${s.regimeLearning.activeRegimeWhyNotReliable} / activeRegimeTopPattern=${s.regimeLearning.activeRegimeTopPattern} / activeRegimeLearningNeed=${s.regimeLearning.activeRegimeLearningNeed}`,
    `Regime Resolved Samples: R2ResolvedSampleSize=${(s.regimeLearning as any).R2ResolvedSampleSize ?? 0} / R2PendingCounterfactual=${(s.regimeLearning as any).R2PendingCounterfactualCount ?? 0} / R3ResolvedSampleSize=${(s.regimeLearning as any).R3ResolvedSampleSize ?? 0} / R3PendingCounterfactual=${(s.regimeLearning as any).R3PendingCounterfactualCount ?? 0} / R6ResolvedSampleSize=${(s.regimeLearning as any).R6ResolvedSampleSize ?? 0} / R6PendingCounterfactual=${(s.regimeLearning as any).R6PendingCounterfactualCount ?? 0} / nextRegimeMaturityAt=${(s.regimeLearning as any).nextRegimeMaturityAt ?? 'N/A'} / regimesNeedingAttributionRecalc=${JSON.stringify((s.regimeLearning as any).regimesNeedingAttributionRecalc ?? [])} / regimeLearningNextAction=${(s.regimeLearning as any).regimeLearningNextAction ?? 'N/A'}`,
    `Counterfactual Metadata Repair: metadataRepairStatus=${s.counterfactualMetadataRepair.metadataRepairStatus} / recoverySourceBreakdown=${JSON.stringify(s.counterfactualMetadataRepair.recoverySourceBreakdown)} / recoveryConfidenceBreakdown=${JSON.stringify(s.counterfactualMetadataRepair.recoveryConfidenceBreakdown)} / recoveredLowCount=${s.counterfactualMetadataRepair.recoveryConfidenceBreakdown.RECOVERED_LOW ?? 0} / coreEligibleRecoveredLowCount=0 / advisoryEligibleRecoveredLowCount=${s.counterfactualMetadataRepair.recoveryConfidenceBreakdown.RECOVERED_LOW ?? 0} / metadataConfidenceHaircutApplied=true`,
    ...
    `Cohort Snapshot: snapshotStatus=${s.cohortSnapshotStatus} / cohortSnapshotAgeSec=${(s.cohortConsistency as any).cohortSnapshotAgeSec ?? -1} / cohortSnapshotMaxAgeSec=${(s.cohortConsistency as any).cohortSnapshotMaxAgeSec ?? 86400} / cohortSnapshotRefreshedAt=${(s.cohortConsistency as any).cohortSnapshotRefreshedAt ?? 'N/A'} / cohortSnapshotRefreshStatus=${(s.cohortConsistency as any).cohortSnapshotRefreshStatus ?? 'UNKNOWN'} / cohortSumMatchesTotal=${s.cohortConsistency.cohortSumMatchesTotal} / ghostRepairReflectedInPulse=${s.cohortConsistency.ghostRepairReflectedInPulse}`,

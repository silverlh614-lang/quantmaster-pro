...
export function collectLearningPulse(now: Date = new Date()) {
  ...
  const cohortSnapshotRefreshedAt = cohortConsistency.lastCohortBackfillRunAt;
  const cohortSnapshotAgeSec = cohortSnapshotRefreshedAt ? Math.max(0, Math.floor((now.getTime() - new Date(cohortSnapshotRefreshedAt).getTime()) / 1000)) : null;
  const cohortSnapshotRefreshStatus = cohortSnapshotAgeSec === null ? 'BACKFILL_JOB_NOT_RUN' : cohortSnapshotAgeSec > cohortSnapshotMaxAgeSec ? 'STALE' : 'OK';
...
export function formatLearningPulseMessage(s: ReturnType<typeof collectLearningPulse>): string {
  ...
  const extra = [
    ...
    `Fresh Shadow Inlet: liveBlocker=${s.freshShadowInlet.liveBlocker ?? 'NONE'} / shadowInletStatus=${s.freshShadowInlet.shadowInletStatus ?? s.freshShadowInlet.nextAction} / queuedNextOpenShadowScan=${String((s.freshShadowInlet as any).queuedNextOpenShadowScan ?? false)} / nextShadowScanAt=${(s.freshShadowInlet as any).nextShadowScanAt ?? 'N/A'} / nextShadowScanReason=${(s.freshShadowInlet as any).nextShadowScanReason ?? 'N/A'} / scanCandidatesToday=${s.freshShadowInlet.scanCandidatesToday} / shadowSignalsToday=${s.freshShadowInlet.shadowSignalsToday} / shadowApprovedToday=${s.freshShadowInlet.shadowApprovedToday} / shadowOrdersCreatedToday=${s.freshShadowInlet.shadowOrdersCreatedToday} / paperFilledToday=${s.freshShadowInlet.paperFilledToday} / counterfactualAllowed=${s.freshShadowInlet.counterfactualAllowed} / executionImpact=${s.freshShadowInlet.executionImpact}`,

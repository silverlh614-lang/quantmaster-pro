...
export function formatLearningPulseMessage(s: ReturnType<typeof collectLearningPulse>): string {
  const totalSuggest = Math.max(Object.values(s.suggest7d).reduce((a,b)=>a+b,0) + s.diagnosticProposals7d, s.suggest.total7d);
  return [formatLearningPulseV3(s.v3), `🩺 Learning Pulse (${s.todayKst})`, `👻 Ghost Portfolio: OPEN ${s.ghost.open} / 7일 close ${s.ghost.closedRecent7d} / closeRate7d ${(s.ghost.closeRate7d*100).toFixed(1)}% / repairCloseRate ${typeof s.ghost.repairCloseRate === 'number' ? `${(s.ghost.repairCloseRate*100).toFixed(1)}%` : 'N/A'} / staleOpenCount ${s.ghost.staleOpenCount}`, `📊 Attribution: ${s.attribution7d.count}/${s.attribution7d.target} (7d) / starvationReason ${s.attribution.starvationReason}${s.attribution.previousStarvationReason ? ` / previousStarvationReason ${s.attribution.previousStarvationReason}` : ''}`, `⚖️ Condition Weights: changed ${s.weights.changedFromDefault} / untouched ${s.weights.untouched} / sunset ${s.weights.sunsetCount}`, `🔔 Suggest 발사: total7d ${totalSuggest} / counterfactual ${s.suggest7d.counterfactual} / ledger ${s.suggest7d.ledger} / kellySurface ${s.suggest7d.kellySurface} / regime ${s.suggest7d.regime} / diagnosticProposals ${s.diagnosticProposals7d} / blocker ${s.suggest.blocker}`, `🧪 Experiment Proposal: active ${s.experimentsActive}`, `🤖 Gemini: 호출 ${s.gemini.callCount}회 / ~${LEARNING_TRADING_DAYS_PER_MONTH}영업일 → 호출률 ${(s.gemini.useRatio*100).toFixed(0)}% / nextScheduledAt ${s.gemini.nextScheduledAt ?? 'N/A'}`, s.flags.length ? `🚩 진단 플래그: ${s.flags.join(', ')}` : '✅ 모든 채널 정상', s.partialFailure ? '⚠️ 데이터 일부 미확인 — 손상/누락 저장소가 있어 가능한 영역만 표시' : ''].filter(Boolean).join('\n');
}

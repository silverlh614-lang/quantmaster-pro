...
export function formatAdminNowDetail(now: Date = new Date()): string {
  ...
  return [
    ...
    `expiresAt=${fmt(latch.latchExpiresAt)} decayLevel=${fmt(latch.latchDecayLevel)} decayPercent=${fmt(latch.latchDecayPercent)} releaseEligibleAt=${fmt(latch.latchReleaseEligibleAt)}`,

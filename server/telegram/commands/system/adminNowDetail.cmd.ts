...
function formatAdminNowDetail(now: Date = new Date()): string {
  ...
  return [
    ...
    `updatedAt=${fmt(snapshot.macroState.updatedAt ?? snapshot.macroState.lastUpdatedAt)}`,
    ...
    `refreshJobEnabled=${fmt(snapshot.macroState.refreshJobEnabled)} refreshJobLastRunAt=${fmt(snapshot.macroState.refreshJobLastRunAt)} refreshBlockedReason=${fmt(snapshot.macroState.refreshBlockedReason)}`,
    `providerUsed=${fmt(snapshot.macroState.providerUsed)} fallbackUsed=${fmt(snapshot.macroState.fallbackUsed)}`,
    `writeSucceeded=${fmt(snapshot.macroState.writeSucceeded)} updatedAtChanged=${fmt(snapshot.macroState.updatedAtChanged)}`,

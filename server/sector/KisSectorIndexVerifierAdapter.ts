...
export async function verifySectorIndexCodeWithKisCurrentPrice(
  ...
  return {
    ...
    ...(result?.clientStatus ? { clientStatus: result.clientStatus } : {}),
    ...(result?.verifyVariantPolicy ? { verifyVariantPolicy: result.verifyVariantPolicy } : {}),
    method: 'GET',
    baseUrlKind: selectedAttempt?.baseUrlKind,
    requestBuilt: selectedAttempt?.requestBuilt,
    requestSent: selectedAttempt?.requestSent,
    httpStatus: selectedAttempt?.httpStatus,
    rtCd: selectedAttempt?.rtCd,
    msgCd: selectedAttempt?.msgCd,
    msg1: selectedAttempt?.msg1,
    outputShape: selectedAttempt?.outputShape,
    indexValueFieldName: selectedAttempt?.indexValueFieldName,
    currentIndex: selectedAttempt?.currentIndex,
    exceptionClass: selectedAttempt?.exceptionClass,
    exceptionMessageSanitized: selectedAttempt?.exceptionMessageSanitized,
    timeoutMs: selectedAttempt?.timeoutMs,
    retryCount: selectedAttempt?.retryCount,
    ...
    attempts: result?.attempts.map((attempt) => ({
      ...
      baseUrlKind: attempt.baseUrlKind,
      requestBuilt: attempt.requestBuilt,
      ...
      indexValueFieldName: attempt.indexValueFieldName,
      currentIndex: attempt.currentIndex,
      exceptionClass: attempt.exceptionClass,
      exceptionMessageSanitized: attempt.exceptionMessageSanitized,

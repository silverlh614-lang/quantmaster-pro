...
/**
 * INVALID_FILENAME_WHITELIST (ADR-0453) — F_INVALID_FILENAME 검증에서 영구 명시 화이트리스트.
 *
...
/**
 * COMMIT_LABEL_ONLY_MARKER (ADR-0453) — B_STALE_INDEX_ENTRY 검증에서 *역사적 사실* 인정 마커.
 *
 * 정책:
...
export function scanAdrFiles(adrDir) {
  ...
  for (const name of readdirSync(adrDir)) {
    ...
    if (!STRICT_NAME_RE.test(name)) {
      // ADR-0453: 화이트리스트 entry 는 invalid 분류 제외하되 byNumber 에 등재 (INDEX 정합 검증 보존)
 ...
 *
 * ADR-0453: mainIndex entry 에 `commitLabelOnly: boolean` 필드 추가 — title 끝에
  ...
  for (const line of lines) {
    ...
    if (inMainIndex) {
      ...
      if (rowM) {
        ...
        // ADR-0453: title 끝에 `(commit-label only, ...)` 또는 `(no file)` 마커 시 historical fact 인정
...
export function validate(scan, parsed) {
  ...
  // ADR-0453: `commitLabelOnly=true` (title 에 `(commit-label only, ...)` 마커) entries 는
  ...
  // ADR-0453: commit-label only entries 카운트 (진단용)
  ...
  const summary = {
    ...
    commitLabelOnlyEntries, // ADR-0453
    nextNumber,

...
export const MASTER_CHECKLIST_STEPS: ChecklistStep[] = [
  ...
  { key: 'relativeStrength', title: "상대 강도 (RS)", desc: "시장 지수 대비 뚜렷한 아웃퍼폼 및 하락장 방어력", icon: Zap, gate: 2 },
...
export const CHECKLIST_LABELS: Record<string, { label: string; description: string; gate: 1 | 2 | 3 }> = {
  ...
  economicMoatVerified: { label: "뚜렷한 경제적 해자(Moat) 보유", description: "독점적 시장 지위, 브랜드 파워, 기술력 등 경쟁사가 쉽게 넘볼 수 없는 진입장벽 존재 여부", gate: 2 },
  ...
  turtleBreakout: { label: "터틀 트레이딩 주요 저항선 돌파", description: "최근 20일 또는 55일 신고가를 경신하며 뚜렷한 추세 추종 신호가 발생했는지 확인", gate: 3 },
  fibonacciLevel: { label: "피보나치 핵심 지지선 반등", description: "상승 후 조정 시 피보나치 0.382 또는 0.618 지점에서 지지를 받고 반등하는지 분석", gate: 3 },
  elliottWaveVerified: { label: "엘리엇 상승 3파/5파 국면 진입", description: "파동 이론상 상승 탄력이 큰 3파동 또는 마지막 분출 구간인 5파동 진입 여부", gate: 3 },

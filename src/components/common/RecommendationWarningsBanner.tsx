/**
 * @responsibility 후보 판정 universe 발굴 경고 영구 표시 — sourceStatus 5-Tier 색상 분기
 */
...
function pickTone(status?: SourceStatus): Tone {
  if (status === 'FALLBACK_SNAPSHOT') {
    return {
      ...
      text: 'text-slate-100/80', Icon: Database, label: '데이터 추정 안내 (스냅샷)',
    };
  }
  if (status === 'ERROR' || status === 'NO_MATCHES') {
    return {
      ...
      text: 'text-red-100/80', Icon: AlertCircle, label: '데이터 추정 안내 (오류)',
    };
  }
  if (status === 'NOT_CONFIGURED') {
    return {
      border: 'border-slate-500/30', bg: 'bg-slate-500/10',
      badge: 'text-slate-300', iconColor: 'text-slate-300',
      text: 'text-slate-100/80', Icon: Info, label: '데이터 추정 안내 (설정)',
    };
  }
  ...
  return {
    ...
    text: 'text-orange-100/80', Icon: AlertTriangle, label: '데이터 추정 안내',
  };
}

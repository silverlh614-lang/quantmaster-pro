...
${preFilledBlock || '      (사전 수집 데이터 없음)'}
      ...
      7. **[판단 기준 - STRONG_BUY, BUY, STRONG_SELL, SELL]**
        ...
        - 시장 환경 조건: VKOSPI 25 미만, BEAR/RISK_OFF 시 STRONG_BUY 금지 및 BUY 비중 축소.
         ...
         - **STRONG_BUY**: [ADR-0005 서버 자동매매 기준과 정렬]
           ① 서버 Gate Score ≥ 9 (11조건 기준, volumeClock 보너스 포함).
           ② RRR ≥ 3.0 (목표가 기준 리스크 대비 보상 3배 이상).
           ③ 진입 레짐이 R1_TURBO / R2_BULL / R3_EARLY (강세 구간) 중 하나.
           ④ MTAS > 3 (월/주/일 타임프레임 정렬).
           ⑤ 기관/외인 5거래일 연속 순매수, 모든 기술적 지표 완벽한 정배열 및 상향 돌파.
           ⑥ 27개 체크리스트 중 25개 이상 만족.
           위 6개 중 하나라도 미충족이면 BUY 로 강등하라. R4_NEUTRAL 이하 레짐에서는 STRONG_BUY 를 아예 부여하지 마라.
         - **BUY**: 서버 Gate Score ≥ 7 (R4_NEUTRAL), RRR ≥ 2.0, 명확한 상승 추세, 주도 섹터 1~2순위 부합, 안정적인 수급 유입(최근 5일 중 3일 이상 순매수), 주요 지지선에서의 반등이 확인되었으며, 27개 체크리스트 중 22개 이상을 만족하는 경우. 단, R5_CAUTION / R6_DEFENSE 레짐에서는 BUY 금지.

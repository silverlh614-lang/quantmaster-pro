...
vi.mock('../../api/kisLiveClient', () => ({ fetchKisLivePrice: vi.fn().mockResolvedValue(null) }));
...
describe('enrichStockWithRealData — OHLCV Naver-first 배선 (장외 기술지표 확보)', () => {
  ...
  it('kisLive 옵션: KIS 공식 현재가가 Naver 보다 우선(on-demand)', async () => {
    const { fetchKisLivePrice } = await import('../../api/kisLiveClient');
    vi.mocked(fetchKisLivePrice).mockResolvedValueOnce({ price: 175000, changeRate: 2 });
    const { enrichStockWithRealData } = await import('./enrichment');
    const result = await enrichStockWithRealData(STUB(), { kisLive: true });
    expect(result.currentPrice).toBe(175000); // KIS 현재가 > Naver 일봉 종가(129)
    expect(result.dataSourceType).toBe('REALTIME');
  });

...
- **4채널 연결 (ADR-0672)** — CH1 signal은 가상 진입·청산, CH2 분석은 같은 종목·진입일의 고정 학습 근거·청산 복기, CH3 정보는 08:45 준비, CH4 시스템은 16:10 성과·일요일 연구를 받는다. 운영 상태는 개인 DM이다. 새 메시지에 목적지를 저장하고 채널별 성공·재시도를 분리한다. 기존 목적지 없는 기록은 DM으로 유지한다.

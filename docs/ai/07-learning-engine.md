...
## 현행 Shadow 관측·학습 (ADR-0666~0669·0672)

현행 경로는 `scanDispatcher → paperExperimentRunner`다. 아래의 옛 Gate·레짐별 학습·F2W 규칙은 기존 구현 유지보수 참고이며, 새 Shadow의 필수 조건으로 적용하지 않는다.
...
- `paperStrategyEvidence/Policy`: 기본 관측과 시점이 검증된 과거 자료를 뉴스 72시간 유무·20일선 위치로 나눠 진입과 보유기간을 판단한다. 동일 유형의 성숙 표본 10건·진입일 3개·비용 차감 평균 양수 조건은 전략 시그널에만 적용한다. 미충족은 WAIT이며 기본 관측 중단 사유가 아니다.
...
- ADR-0678: `paperObservationFeatures`는 같은 스냅샷의 완료 일봉·현재가·별도 갱신 재무 캐시로 26개 연구 값을 진입 시 고정한다. 현재 수집률과 진입 고정 D1/D3/D5 구간 비교를 기본 관측 화면에 표시한다. 재무 결산기간·연결/별도·확인 시각을 보존하며 과거 관측에 소급 보충하지 않는다. 재무 갱신은 한 번에 최대 8종목, 성공 24시간/실패 1시간 간격이고 가격 수집을 기다리게 하지 않는다. 추가 값은 진입 필수 조건이나 매매 정책으로 사용하지 않는다.

...
- 2026-05-26 · Patch-SECTOR-INDEX-VERIFIER-TSC-SERVER-FIX-001 (Patch #1209/#1212 회귀: SectorIndexVerifier 의 SECTOR_INDEX_MARKET_CLOSED/_OBSERVE_ONLY 리터럴을 promotionCoveragePolicy.reason·promotionReadiness.reason union 에 미반영 + 테스트 optional 미처리로 tsc --noEmit -p tsconfig.server.json 5건 에러 — GitHub CI 가 tsc 미실행이라 미검출. reason union 2개에 신규 리터럴 추가 + 테스트 optional chaining; npm run lint(tsconfig+tsconfig.server) clean 복구; executionImpact=NONE)

...
- 2026-05-28 · Patch-WIRE-Selected-Candidate-Actual-Row · signalScanner supply-carry, injectPerSymbolSupplyContext 후보전체 per-symbol flow→단일 aggregate by-symbol map(buildInvestorFlowBySymbolCarryMap, MAX_SNAPSHOTS=20 cap 제거) + index.ts→persistScanResults→gate1 forensic collector supplyRouterResult.bySymbol 로 직접 thread(snapshot retention/freshness 비의존, 결정론적 carry) → gateScoreEligible/semanticAvailable ~21→데이터있는 후보전체(~46), program placeholder partial 허용, stats.investorFlowBySymbolCount 진단, 매매/threshold/scoring/order 무변경, 신규 carry test 7(inject 4+collector 3), marketSignal=false/usableForGate=false/executionImpact-NONE(SHADOW_ONLY)

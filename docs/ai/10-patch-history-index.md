...
- 2026-05-27 · Patch-Gate1-Threshold-Diagnosis-Doc · diagnosis-only, docs/gate1-threshold-diagnosis.md 추가 — Gate1 통과율 저하 구조적 진단(임계값 70 자체 아님): provider/수급 UNKNOWN 시 SUPPLY_CONFLUENCE(8)+INVESTOR_FLOW(8)=16점 포기-positive 증발(불변식 #4/#6 정합 0점), 점수 천장 108→92 붕괴로 70=76%컷화, SUPPLY_PROVIDER_UNKNOWN 계열(TRUE_GATE1_REJECTION 아님), 기설계-미연결 해법=buildActiveComponentRequiredScorePolicy coverage-adjusted requiredScore(floor50/ceiling70 DRY_RUN), 실측분포 미확보(라이브 provider/스캔데이터 없음)→운영 /scan_blockers gate·gate1 calibration audit 필요, 코드/requiredScore=70/매매로직 0줄 변경, executionImpact-NONE

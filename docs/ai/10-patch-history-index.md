...
- 2026-05-25 · ADR-0526 (CandidateGateEvaluationView SSOT — Phase 1 구현: per-candidate Gate0/1/2/3 판단 정본을 persist 시점에 단일 도출(candidateGateViews/candidateGateAggregate + candidateGate2Confluence/candidateGate3Closure 스냅샷, gate2 캐시 미사용→결정론)하고 formatter 6종(gateUx/scanBlockersGate2/scanBlockersGate3/gate2ExternalRefresh/snapshotBundle gate2 합성)을 traces 재추론·buildGate2ConfluenceSummary·buildGate3RuntimeClosureSummary 재실행에서 View read 로 재바인딩; gate2 PASS/FAIL status 를 render-time 캐시 우선(cacheExternal??scanExternal)에서 스캔-시점 정본으로 전환(의도된 변경)·캐시 외부데이터는 "참고(캐시)" display-only 강등(status 비-override); legacy read-guard scripts/check_gate_view_boundary.js 신규(분류(b) 6파일에서 gate verdict 직접 채굴+3종 정본 빌더 재실행 차단, GATE-VIEW-EXEMPT 는 Phase2 execution input 한정) + validate:all·precommit 체인 가산; 음성 주입→EXIT1 검출 확인; execution/PositionPolicy/autoTradeEngine/kisClient 0줄(Phase2 격리)·LIVE 본체 0줄; gate3 byte-equivalent·gate2 의도 변경; stash 대조로 신규 회귀 0(baseline 7파일 동일); executionImpact=NONE)

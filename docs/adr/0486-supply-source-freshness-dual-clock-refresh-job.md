...
## Refresh Job Policy
ADR-0486 refresh status is diagnostic/dry-run first. It can recommend refresh, mark dry-run refresh evidence, skip non-trading days, or report provider failure, but it does not call live execution paths and does not fetch inside `/scan_blockers` formatters.

Refresh failure is try/catch isolated and must not stop scan, Shadow Learning, Runtime Pipeline Audit, or Telegram commands.

## Guardrails
ADR-0486 keeps these invariants:
...
3. ADR-0485 Semantic Net-Buy Normalizer
...
Runtime Pipeline Audit may count ADR-0486 as diagnostic evidence only; it must not alter rollout status.

## Consequences
`/scan_blockers` can show `ADR-0486 SupplyFreshness: STALE | oldest=5d | affected=FSS/short/credit | impact=NONE`, while `/supply_health_detail` can show each cache clock and source clock separately. ADR-0477 and ADR-0485 may lower confidence when source data is stale, but stale/provider issues remain separated from bearish market signal.

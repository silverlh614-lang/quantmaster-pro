# ADR-0640 — 레짐 전환 알림 정합화: 진동 억제 + 정직한 전환 사유 + 무변화 라벨
...
- **allowedFiles**: `server/trading/regime/regimeTransitionNotice.ts`(신규) · `regimeBridge.base.ts`(import + 알림 블록 + 모듈 상태/reset) ·
  `regimeTransitionNotice.test.ts`(신규) · 본 ADR · `INDEX.md`(0640→0641) · `10-patch-history-index.md`(1줄) · `.env.example`(ENV 1줄).
...
- **rollbackPlan**: `REGIME_NOTIFY_MIN_DWELL_MIN=0` 로 진동 억제 즉시 비활성(legacy 발송 빈도 복원). 문구 개선은 revert 1커밋.

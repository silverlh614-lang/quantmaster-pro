...
| B8 | 0161+0162+0163 PositionSizingEngine | `server/trading/sizing/positionSizingEngine.ts` | 2026-05-02 | PARTIAL | P0 | **Phase1 (0161) + Phase2D 메인 (0162) + Phase2D Extension 3 경로 (0163) wiring 완료** — buyListLoop 3 호출 (메인 + PRE_BREAKOUT_FOLLOWTHROUGH + PRE_BREAKOUT 30%) + intradayLoop 1 호출 = 총 4 진입 경로 모두 본 모듈 호출. ENV `POSITION_SIZING_ENGINE_SHADOW_APPLY=true` (default OFF) + `sizingSource` marker 영속. **잔여 4 PR**: LIVE 활성화 (`_LIVE_ENABLED=true` ENV, P0 후속, SLA 2026-05-23) + drawdown 추적 (peakEquity 영속) + lossStreak 외부 SSOT + universe/sectorWeight 결합. SLA 만기일: **2026-05-23** (LIVE 활성화 결정) |

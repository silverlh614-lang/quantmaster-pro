...
| A3 | 0006 emitFullCloseAttribution | `server/persistence/attributionRepo.ts` | PARTIAL | P0 | 8/13 wired (hardStopLoss/legacyTakeProfit/cascadeFinal/ma60DeathForceExit/trailingStop/trancheTakeProfitLimit + ocoCloseLoop STOP_FILLED/PROFIT_FILLED). **5 잔여 (2026-05-01 audit)**: r6EmergencyExit (P0 전량) / atrDynamicStop (P0 전량) / bearishDivergenceExit (P1 전량) / cascadeHalf (P1 부분 50%) / euphoriaPartialExit (P1 부분). PR-A3-1 r6+atr (P0) → PR-A3-2 bearishDiv (P1) → PR-A3-3 cascadeHalf+euphoria (P1) 3 PR 분할 권장 |

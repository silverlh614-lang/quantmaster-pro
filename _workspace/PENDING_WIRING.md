...
| C19 | 0434 actual candle/cache lookup hardening | `server/learning/provisionalShadowPriceProvider.ts` (cache-only stub) + `server/learning/counterfactualShadowPriceProviderAdapter.ts` | 2026-05-07 | INFRASTRUCTURE_ONLY | P2 | **사용자 명시 부채 등록 (2026-05-07, ADR-0433 후속 세션 종료 시)**. ADR-0429 priceProvider 가 default cache-only 라 모든 horizon DATA_UNAVAILABLE/PENDING — 실제 INTRADAY_CANDLE_CACHE / DAILY_CANDLE_CACHE / MARKET_DATA_CACHE / READ_ONLY_QUOTE wiring 필요. 사용자 명시 *"부채로 남기고 세션 종료"*. 본 PR scope 외 — 외부 API 호출 비용·quota 정책 + ADR-0431/0432/0433 분석 정확도 격상 양 효과를 함께 검토 후 별도 PR. SLA 만기 2026-09-04 (P2 120일). |

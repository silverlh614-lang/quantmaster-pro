...
| B8 | 0161+0162 PositionSizingEngine | `server/trading/sizing/positionSizingEngine.ts` | 2026-05-02 | PARTIAL | P0 | **PR-Sizing-Engine-Phase1 (ADR-0161) 8 파일 신설 + Phase2D (ADR-0162) 옵션 D SHADOW wiring 완료** — `buyListLoop.ts` 메인 buyList wiring + ENV `POSITION_SIZING_ENGINE_SHADOW_APPLY=true` (default OFF) + `sizingSource` marker 영속. **잔여 5 PR**: PRE_BREAKOUT_FOLLOWTHROUGH/PRE_BREAKOUT/INTRADAY 3 곳 확장 (P1) + LIVE 활성화 (`_LIVE_ENABLED=true` ENV, P0 후속) + drawdown 추적 + lossStreak 외부 SSOT + universe/sectorWeight 결합. SLA 만기일: **2026-05-23** (LIVE 활성화 결정) |

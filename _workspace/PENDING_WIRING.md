...
| A6 | 0123/0124 recommendationTracker | `server/learning/recommendationTracker.ts` | 2026-05-02 | BLOCKED | P1 | 학습 SSOT 변경 회귀 위험 — monthlyStats.winRate fill 단위 격상 (현재 trade 단위 WIN/LOSS 만, fill 수준 BE 분류 미반영). fill 수준 BE 분류에 필요한 fill outcome 데이터 누적 후 격상 (재노출 2026-07-01) |
...
| A11 | 0173 §5 LearningFreshnessGuard | `server/learning/learningFreshnessGuard.ts` | 2026-05-03 | BLOCKED | P1 | **PR-Shadow-Learning-Persistence-Phase1 (2026-05-03, ADR-0173)** — 호출자 0건 dead code. Phase 3 ReflectionInjectionBus 결합 후 활성화: `mainReflection` / `scoreBuyCandidate` / `condition weight engine` / `position sizing adjustment` 5 lesson 타입 (recentReflections / conditionLessons / biasHeatmap / counterfactualLessons / gateAttributionLessons) 에 `applyFreshnessDecay` 호출 wiring + `LEARNING_FRESHNESS_GUARD_ENABLED=true` 명시 활성화. Phase 3 ReflectionInjectionBus 결합·검증 후 활성화 — 선행 Phase 미착수 동안 호출자 0건 유지 (재노출 2026-07-01). |
...
| ID | ADR | 모듈 | 등재일 | 상태 | 우선순위 | 차단 사유 / 다음 액션 |
|----|-----|------|--------|------|----------|----------------------|
| B2 | 0085 Slot Sizing | `server/trading/slotSizing.ts` | 2026-05-02 | BLOCKED | P1 | 사용자 결정 대기 — `perSymbolEvaluation.ts:902` `evaluateSlotSizing()` wiring + intraday sizing 동일 분기 — `SLOT_CAPITAL_WEIGHTED_SIZING_ENABLED=true` 명시 활성화 + 1주 검증 후 (재노출 2026-07-01) |
| B3 | 0001 preflight Phase B | `server/trading/signalScanner/preflight.ts` | 2026-05-02 | BLOCKED | P1 | LIVE 회귀 위험 격리 — 매크로 게이트 본체 교체 (signalScanner.ts:runPreflight() inline → preflight.ts 위임). signalScanner 분해 Phase B 후속 — Phase B 리팩토링·검증 후 위임 전환, LIVE 매크로 게이트 본체라 단독 선행 불가 (재노출 2026-07-01) |
...
| B5 | 0117/0128 entryRevalidationStep | `server/trading/signalScanner/revalidationSteps/entryRevalidationStep.ts` | 2026-05-02 | BLOCKED | P1 | 사용자 결정 대기 — DATA_HOLD 분기 SSOT 위임 격상 (현재 진단 디테일 보존 vs 일관성 trade-off, 재노출 2026-07-01) |
...
| B14 | 0414 PriceCorrection Stage 2 Shadow | `server/trading/signalScanner/priceCorrectionEngine.ts` | 2026-05-06 | BLOCKED | P1 | **Stage 2a 완료 (PR-PriceCorrection-Stage2a, 2026-06-18, ADR-0623)** — `PRICE_CORRECTION_SHADOW_ENABLED` ENV(default OFF·`isPriceCorrectionDisabled` 우선) 신설 + Seam A: dead 모듈(`evaluatePriceIntegrity`/`evaluatePriceCorrection`)을 live diagnostics 로 전환(`ScanSummary.priceIntegrity`/`.priceCorrection` 집계 채움 → `/scan_blockers` 종목별 가시화). byte-equivalent(ENV OFF)·LIVE 무접촉·외부 API 0·원본 mutate 0. **잔여 (Stage 2b P1)**: shadow corrected 치환 — `kisIntradayCorrectionStep` 시그니처 확장 + SHADOW 분기 reCheckQuote 복제본에 corrected price/prevClose 주입 + 4중 게이팅(`shadowEnabled && stockShadowMode && usableForShadow && confidence≥0.5`) + buildBuyTrade lineage 스탬프 + 회귀 #5/#6. SLA 만기 2026-06-20 — Stage 2a wiring 으로 status 격상. 활성화 절차: Stage 2b 후 운영자 1주 SHADOW 검증 → Stage 3(B15) 진입. ADR-0414 정책 — Stage 순차 게이트, Stage 2b 는 운영자 검증 후 진행 (재노출 2026-07-01). |
...
| C6 | 0136 PR-1 후속 | `server/trading/regimeBridge.ts` | 2026-05-02 | BLOCKED | P1 | regime 의사결정 회귀 격리 — `passiveActiveBoth=null → R3_EARLY 트리거 보수화` wiring. R3_EARLY forward-outcome 데이터 누적·검증 후 wiring (재노출 2026-07-01) |
...
| ID | ADR | 모듈 | 등재일 | 상태 | 우선순위 | 차단 사유 / 다음 액션 |
|----|-----|------|--------|------|----------|----------------------|
| D1 | 0098 ConfluenceMeter | `src/components/common/ConfluenceMeter.tsx` | 2026-05-02 | DECIDED_NOT_WIRING | P1 | **UI 임베드 보류 결정 (PR-PendingWiring-SLA-Triage, 2026-07-01)** — 실사용처 0건·외부 차단 없음, go-live 준비와 무관한 순수 UI 가시성 작업이라 보류. UI 가시성 패스 재개 시 재등재. 원 계획: DiscoverWatchlistPage Top 3 시범 임베드 + VerdictCard.Evidence 안 자식 |
...
| D3 | 0097 VerdictCard | `src/components/watchlist/WatchlistCard.tsx` | 2026-05-02 | DECIDED_NOT_WIRING | P1 | **UI 마이그레이션 보류 결정 (PR-PendingWiring-SLA-Triage, 2026-07-01)** — variant='verdict' 실사용처 0건·외부 차단 없음, 50+ 컴포넌트 점진 마이그레이션은 UI 가시성 패스 재개 시 재등재. 원 계획: WatchlistCard 마이그레이션 (variant='verdict' 점진 도입) |
...
| D8 | 0504 positionsRouter REAL mode wiring | `server/routes/autoTrade/positionsRouter.ts` | 2026-05-12 | BLOCKED | P1 | UI 가시성 — getOpenPositions SSOT 위임 + REAL mode `fetchKisHoldings` wiring (응답 schema breaking change 위험으로 별도 PR 분리, ADR-0504). REAL mode 운영 전환·검증 후 별도 PR 진행 (재노출 2026-07-01) |
...
| 2026-07-01 | PR-PendingWiring-SLA-Triage | H_SLA_FAIL 8건(A2/A6/B2/B3/B5/C6/D1/D3) + H_SLA_WARN 3건(A11/B14/D8) 일괄 triage — go-live 사전 점검 중 validate:all EXIT 1 해소 (문서 전용·코드 0줄). A2/A6/B5/B14 PARTIAL→BLOCKED + A11/B2/B3/C6/D8 INFRASTRUCTURE_ONLY→BLOCKED — 각 reason 에 실제 차단 사유(운영자·사용자 결정 / fill·forward-outcome 데이터 누적 / Phase·Stage 검증 후) 명문화 + 재노출 2026-07-01 마커(I-check clock 리셋). D1/D3 INFRASTRUCTURE_ONLY→DECIDED_NOT_WIRING — UI 임베드 보류 결정(실사용처 0건·외부 차단 없음, UI 가시성 패스 재개 시 재등재). SLA 우회 ENV(WIRING_SLA_DISABLED/GRACE_DAYS) 불사용. 통계 표 무변경(status 전환만, 우선순위 카운트 유지). validate:pendingWiring FAIL 8→0. |

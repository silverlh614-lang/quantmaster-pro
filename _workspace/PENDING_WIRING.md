...
| A10 | 0173 §1 MissedLearningQueue | `server/learning/missedLearningQueue.ts` | 2026-05-03 | PARTIAL | P1 | **PR-Shadow-Learning-Phase2b2 (2026-05-03, ADR-0176) — wiring 완료**: scheduleGuard hook + 5 학습 cron `enqueueOnSkip: {}` 옵션 + 신규 `missed_learning_replay` cron (KST 평일 09:30 + ScheduleClass='TRADING_DAY_ONLY' + ENV gate). **잔여 (P1, Phase 3 후속)**: `replayMissedLearningJobs` jobName → 실제 함수 매핑 dispatcher (현재 mock dispatcher plumbing only) + 운영자 ENV `MISSED_LEARNING_QUEUE_ENABLED=true` 명시 활성화. SLA 만기 2026-06-17. |
...
| A15 | 0176 MissedLearningReplay cron | `server/scheduler/learningJobs.ts` | 2026-05-03 | INFRASTRUCTURE_ONLY | P1 | **PR-Shadow-Learning-Phase2b2 (2026-05-03, ADR-0176)** — replay cron 등록 완료 (`30 0 * * 1-5` UTC = KST 평일 09:30 + ScheduleClass='TRADING_DAY_ONLY'), 호출자 1건 cron 만 + ENV `MISSED_LEARNING_QUEUE_ENABLED` default OFF. **잔여 wiring (Phase 3 후속)**: `replayMissedLearningJobs` jobName → 실제 함수 매핑 dispatcher 추가 (현재 ADR-0173 §1 plumbing only — status 'PENDING' → 'REPLAYED' 전이만 mock). Phase 3 LIVE 활성화 시 5 학습 cron silent skip → enqueue → 다음 영업일 09:30 replay → dispatcher 가 실제 함수 호출. SLA 만기 2026-06-17. |

### B. 매매 본체 / 사이징

| ID | ADR | 모듈 | 등재일 | 상태 | 우선순위 | 차단 사유 / 다음 액션 |
|----|-----|------|--------|------|----------|----------------------|
| B1 | 0085 TwoBar Confirmation | `server/trading/twoBarConfirmation.ts` | 2026-05-02 | DECIDED_NOT_WIRING | P0 | **PR-P0-Activation (2026-05-06) 완료** — default OFF → ON 전환. `isBepTwoBarLiveEnabled()` `=== 'true'` → `!== 'false'` 패턴 격상 (ADR-0157 정확 비교). 회귀 발견 시 ENV `BEP_TWO_BAR_LIVE_ENABLED=false` 1줄 즉시 롤백. SHADOW 1주 검증 완료 (PR-B1-1 머지일 2026-05-01 ~ 활성화일 2026-05-06) + 사용자 SHADOW only 운영 (LIVE 미진입) 회귀 위험 0. P0 SLA 만기 2026-05-23 17일 전 충족. |
...
| B11 | 0173+0183 ShadowLearningOnlyScan wiring | `server/trading/shadowLearningOnlyScan.ts` | 2026-05-03 | DECIDED_NOT_WIRING | P0 | **PR-P0-Activation (2026-05-06) 완료** — default OFF → ON 전환. `isShadowLearningOnBlockedDaysEnabled()` `=== 'true'` → `!== 'false'` 패턴 격상 (ADR-0157 정확 비교). 차단된 날 (R6_DEFENSE / VIX panic / FOMC DAY / SELL_ONLY) Shadow learning 표본 자동 누적 즉시 시작 — Phase 4-B Dashboard 6 카드 실제 데이터 노출. **LIVE 매매 본체 영향 0** (ADR-0183 §"안전 invariant 7종" — SHADOW only 격리, allowRealOrder=false literal type + runtime throw 2중 강제). **잔여 (P2)**: Phase 3 Stage B (replayMissedLearningJobs dispatcher) + Stage C (ReflectionInjectionBus). 회귀 발견 시 ENV `SHADOW_LEARNING_ON_BLOCKED_DAYS_ENABLED=false` 1줄 즉시 롤백. P0 SLA 만기 2026-05-24 18일 전 충족. |
...
| B14 | 0414 PriceCorrection Stage 2 Shadow | `server/trading/signalScanner/priceCorrectionEngine.ts` | 2026-05-06 | INFRASTRUCTURE_ONLY | P1 | **ADR-0414 Stage 1 완료 (본 PR)** — Stage 1 Read-Only Mode 인프라만. **잔여 (Stage 2 P1)**: `PRICE_CORRECTION_SHADOW_ENABLED=true` ENV 도입 + Shadow scan 분기 wiring (signalScanner Shadow 분기에서 `evaluatePriceCorrection` 호출 + `usableForShadow=true` 시 corrected 값 사용). signalScanner / scanDiagnostics 진입부 wiring + ScanSummary 옵셔널 필드 영속. SLA 만기 2026-06-20 (P1 45일). 활성화 절차: 운영자 1주 SHADOW 검증 → 만족 시 활성화 → Stage 3 진입. |
| B15 | 0414 PriceCorrection Stage 3 Live | `server/trading/signalScanner/priceCorrectionEngine.ts` | 2026-05-06 | INFRASTRUCTURE_ONLY | P0 | **ADR-0414 Stage 1 완료 (본 PR)** — Stage 1 Read-Only Mode 인프라만. **잔여 (Stage 3 P0)**: `PRICE_CORRECTION_LIVE_ENABLED=true` ENV 도입 + 운영자 명시 승인 + signalScanner / 27조건 / Gate 1~3 wiring + `usableForLiveEntry=true` 강제 해제 (현재 Stage 1 항상 false). LIVE 매매 본체 결합 — 회귀 위험 *큼*. SLA 만기 2026-05-27 (P0 21일, Stage 2 검증 후). 활성화 절차: Stage 2 SHADOW 1주 + corrected vs original 백테스트 비교 + 운영자 명시 승인 → ENV 활성화 → 운영 데이터 1주 검증. |
...
| C19 | 0434 actual candle/cache lookup hardening | `server/learning/provisionalShadowPriceProvider.ts` (cache-only stub) + `server/learning/counterfactualShadowPriceProviderAdapter.ts` | 2026-05-07 | DECIDED_NOT_WIRING | P2 | **ADR-0439 (= 사용자 명시 ADR-0434, 본 commit) 머지 완료 — provisional 측 4-tier (INTRADAY/DAILY/MARKET_DATA/READ_ONLY_QUOTE) wiring 활성화**. counterfactualShadowPriceProviderAdapter 헬퍼 (`readYahooSnapshotPoint`/`parseYahooChartBody`/`normalizeYahooSymbol`/`closestPointAtOrAfter`/`latestPoint`/`ParsedYahooPoint`) export 추가 (본체 0줄 변경) + `lookupCachedPrice` 결정 트리 SSOT (ENV→SCAN_SNAPSHOT→horizon target time→INTRADAY/DAILY 우선→MARKET_DATA→READ_ONLY_QUOTE→null) + ENV `PROVISIONAL_CACHE_LOOKUP_DISABLED` default OFF + maxExternalLookups default 0 그대로 (cache-only 정책 보존, 외부 API 호출 도입 0). 회귀 38 신규 + 인접 111/111 무회귀. 외부 API 호출 활성화 (`maxExternalLookups>0`) 는 별도 ADR. 부채 종결. |

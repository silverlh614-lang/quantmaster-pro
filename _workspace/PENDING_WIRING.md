...
| B9 | 0166+0167 RegimeExposureBudget | `server/trading/sizing/regimeExposurePolicy.ts` | 2026-05-02 | PARTIAL | P2 | **Phase A+B+AccurateExposure wiring 완료** (ADR-0166+0167) — 7 레짐 매트릭스 + 4 wiring + 정확 산출 SSOT (`computeCurrentEquityExposure` 활성 trade 보유원가 합산 + ENV `POSITION_SIZING_ACCURATE_EXPOSURE_ENABLED=true` default OFF). audit-PR-520 §M1 수리 완료. **잔여 4 PR (P2)**: currentPriceMap 시가 평가 (KIS 호출) + isAddOnBuy 추매 진입점 (trancheExecutor) + UI 출력 + 매크로 R0~R6 자동 분류. 활성화 절차 (ADR-0166 §7 + ADR-0167 §7): SHADOW APPLY → EXPOSURE BUDGET → ACCURATE EXPOSURE → LIVE 4단계. |

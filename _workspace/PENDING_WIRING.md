...
| B12 | 0168b/0184 emergencyDataQualityGuards | `server/dataQuality/emergencyDataQualityGuards.ts` | 2026-05-05 | PARTIAL | P1 | **PR-B12-A (2026-05-05, ADR-0184) site 1+2 wiring 완료** — site 1 (universeScanner 3 함수 master guard, ENV `EMERGENCY_MASTER_GUARD_SCAN_ENABLED=true` default OFF, 운영자 활성화 결정 대기) + site 2 (watchlistRepo.saveWatchlist invalid KRX code filter, default ON, `0070X0` 같은 결함 영구 차단). ENV 헬퍼 SSOT 2종 신규 (`isEmergencyMasterGuardScanEnabled` + `isEmergencyWatchlistCodeGuardEnabled`). **잔여 P1 wiring (B12-B 별도 PR)**: site 3 `buyPipeline.createBuyTask` KRX code sanity 가드 / site 4 `pipelineHelpers.evaluateStage1Filter` strict 분기 (`classifyStage1RejectStrict` DATA_MISSING_* 분리). SLA 만기 2026-06-19 (P1 45일). |

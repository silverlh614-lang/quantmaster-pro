...
4. HOLIDAY, 장전/장후, providerIssue는 SourceSnapshot을 바꾸지 않는다. R1~R6 레짐과 Kelly는 LIVE·PAPER·SHADOW 운영 정책에서 폐기한다.
...
## 새 Shadow 모델의 단순성 — 사용자 지시 2026-09-13

초기 QMP의 과도한 조건·제약 누적으로 시스템이 작동하지 않던 문제를 반복하지 않는다.

- 사용자 추가 승인(2026-09-13): LIVE·PAPER에서도 레짐과 Kelly를 완전히 제거한다. 구 레짐 자동 진입, R6 긴급청산·레짐별 손절 조정까지 폐기하되 가격 기준 손절·목표·OCO·체결·자금·중복 안전은 보존한다.
- 사용자 운영 방향(2026-09-13): 기존 KIS 인증·연결·호가·가상체결 경로를 유지한다. 당장 LIVE로 전환하지 않으며 관측·추천·시그널·학습의 오류를 하나씩 고친다. 후보 발굴 정리가 브로커 교체, 체결 방식 변경, LIVE 자동 승격으로 확대되어서는 안 된다.
- 레짐은 매매 판단에서 우회하는 데 그치지 않고 현재 시장 분류·전이·경고·정책 가중치 갱신에서 제거한다. 과거 원장과 순수 재현 계산은 이력 호환용으로만 보존하며 운영 경로에 복원하지 않는다.
- 현행 Shadow는 독립 기본 관측과 뉴스·20일선 유형별 성과 전략이다. 기존 Gate·70점·레짐·Kelly·운영자 승인 조건을 이 경로에 복원하지 않는다.
- 매수 판단이 대기해도 기본 관측과 D1·D3·D5 성과 누적은 계속한다. 전략의 최소 표본·기대수익 조건을 기본 관측의 자격 조건으로 옮기지 않는다. 실제 가격·시각 검증과 중복 방지, 실주문 안전 경계는 유지한다.
- 새 지표는 먼저 개별 연구로 검증한다. 연구 항목을 자동으로 필수 조건에 추가하거나 여러 조건의 동시 충족을 계속 누적하지 않는다. 변경은 독립 효과와 관측·시그널 발생에 미치는 영향을 함께 확인해 단순한 경로를 유지한다.
- 무신호는 대기 사유와 관측·성과 누적 상태를 함께 확인한다. 정상 대기와 엔진·수집 장애를 구분하고, 무조건 매수하게 만들거나 임계값·승인 단계를 더 붙이는 방식으로 해결하지 않는다.
